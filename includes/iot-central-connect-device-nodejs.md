...
A korábban letöltött Node.js Microsoft Azure IoT SDK másolatában nyissa meg az *Azure-IoT-SDK-Node/Device/Samples/PnP/pnpTemperatureController.js* fájlt egy szövegszerkesztőben.
...
* Létrehoz egy `client` objektumot, és beállítja a `dtmi:com:example:TemperatureController;2` modell azonosítóját a kapcsolatok megnyitása előtt. IoT Central a modell AZONOSÍTÓját használja az eszközhöz tartozó sablon azonosításához vagy létrehozásához. További információ: [eszköz hozzárendelése egy eszköz sablonnal](../articles/iot-central/core/concepts-get-connected.md#associate-a-device-with-a-device-template).
* Parancssori kezelőket hoz létre három parancshoz.
* Elindítja az egyes termosztát-összetevők hurkoit, hogy 5 másodpercenként elküldjék a hőmérséklet-telemetria.
* Elindít egy hurkot az alapértelmezett összetevő számára a munkakészletek méretének telemetria való elküldéséhez 6 másodpercenként.
* Az `maxTempSinceLastReboot` egyes termosztát-összetevők tulajdonságának elküldése.
* Az eszköz információinak tulajdonságainak küldése.
* Az írható tulajdonságok kezelőit hozza létre a három összetevőhöz.
...
A `sendTelemetry` függvény azt mutatja, hogy az eszköz hogyan küldi el a hőmérséklet telemetria IoT Central. Az összetevőkből származó telemetria egy nevű tulajdonságot adhat hozzá `$.sub` az összetevő nevével:
...
A `main` metódus egy nevű Helper metódust használ a `helperCreateReportedPropertiesPatch` Tulajdonságok frissítési üzeneteinek létrehozásához. Ez a metódus egy opcionális paramétert használ a tulajdonságot küldő összetevő megadásához.:
...
A `main` metódus a következő módszert használja az írható tulajdonságok frissítésének kezeléséhez IoT Centralból. Figyelje meg, hogy a metódus hogyan hozza létre a választ a verzió és az állapotkód szerint:
...
A `main` metódus a következő módszerekkel kezeli a IoT Central parancsait:
...
A minta alkalmazás futtatásához nyisson meg egy parancssori környezetet, és navigáljon az *Azure-IOT-SDK-Node/Device/Samples/PnP* mappába, amely tartalmazza az *pnpTemperatureController.js* -mintát.
...
Az alábbi kimenetben az eszköz regisztrálása és a IoT Centralhoz való csatlakozás látható. A minta ezután elküldi a `maxTempSinceLastReboot` tulajdonságot a két termosztát-összetevőtől a telemetria küldésének megkezdése előtt:
...
Láthatja, hogy az eszköz hogyan válaszol a parancsokra és a tulajdonságokra. A `getMaxMinReport` parancs az `thermostat2` összetevőben található, a `reboot` parancs az alapértelmezett összetevőben található. A `targetTemperature` "thermostat2" összetevő írható tulajdonsága be lett állítva:
