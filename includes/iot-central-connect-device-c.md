...
ms.locfileid: "106491207"
---
## <a name="prerequisites"></a>Előfeltételek

A cikkben szereplő lépések végrehajtásához az **egyéni alkalmazás** sablonnal létrehozott Azure IoT Central alkalmazásra van szükség. További információért lásd az [alkalmazás létrehozását bemutató rövid útmutatót](../articles/iot-central/core/quick-deploy-iot-central.md). Az alkalmazást a 2020. július 14-én vagy azt követően kell létrehozni.
...
A korábban letöltött Microsoft Azure IoT SDK fájljának másolatában nyissa meg az *Azure-IoT-SDK-c/iothub_client/samples/pnp/pnp_temperature_controller/pnp_temperature_controller. c* és az *Azure-IoT-SDK-c/iothub_client/samples/pnp/pnp_temperature_controller/pnp_thermostat_component. c* fájlokat egy szövegszerkesztőben.
...
A *pnp_temperature_controller. c*-ben a `main` függvény először a `CreateDeviceClientAndAllocateComponents` következőket kéri:
...
* Létrehoz egy kezelőt a parancsokhoz a hőmérséklet-vezérlő összetevőben.
* Létrehoz egy kezelőt a tulajdonságok frissítéseihez a hőmérséklet-vezérlő összetevőben.
* Létrehozza a két termosztát-összetevőt.

A `main` következő függvény:

* Az összes összetevőhöz tartozó kezdeti tulajdonságértékek jelentése.
* Elindít egy hurkot, hogy telemetria küldjön az összes összetevőtől.
...
A alkalmazásban `pnp_thermostat_component.c` a `PnP_ThermostatComponent_SendTelemetry` függvény azt mutatja be, hogy az eszköz hogyan küldi el a hőmérsékleti telemetria egy összetevőtől a IoT Central:
...
A alkalmazásban `pnp_thermostat_component.c` a `PnP_TempControlComponent_Report_MaxTempSinceLastReboot_Property` függvény egy `maxTempSinceLastReboot` tulajdonság frissítését küldi el az összetevőtől IoT Central:
...
A alkalmazásban `pnp_thermostat_component.c` a `PnP_ThermostatComponent_ProcessPropertyUpdate` függvény az írható tulajdonságok frissítéseit IoT Central:
...
A-ben `pnp_thermostat_component.c` a `PnP_ThermostatComponent_ProcessCommand` függvény kezeli a IoT Central nevű parancsokat:
