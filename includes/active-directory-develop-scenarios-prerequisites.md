---
title: fájl belefoglalása
description: fájl belefoglalása
services: active-directory
author: jmprieur
manager: CelesteDG
ms.service: active-directory
ms.topic: include
ms.workload: identity
ms.date: 09/25/2020
ms.author: jmprieur
ms.custom: include file
ms.openlocfilehash: 79952ad04df4780e1b30eb9cf5c7a35f49cc34c2
ms.sourcegitcommit: f28ebb95ae9aaaff3f87d8388a09b41e0b3445b5
ms.translationtype: MT
ms.contentlocale: hu-HU
ms.lasthandoff: 03/29/2021
ms.locfileid: "91257069"
---
Ha még nem ismeri a OAuth 2,0 és az OpenID Connect szolgáltatást, vagy akár csak most ismerkedik a Microsoft Identity platformon, az alábbi cikkeknek magasnak kell lenniük az olvasási listán.

Bár az első gyors útmutató vagy oktatóanyag elvégzése előtt nem szükséges az olvasás, a platform szerves részét képező témaköröket fedi le, és a velük való ismeretek segítenek az Ön útvonalán az összetettebb forgatókönyvek kialakítása során.

:::row:::
    :::column:::
        **Hitelesítés és engedélyezés**

        - [Hitelesítési alapok](../articles/active-directory/develop/authentication-vs-authorization.md)
        - [Azonosító jogkivonatok](../articles/active-directory/develop/id-tokens.md)
        - [Hozzáférési jogkivonatok](../articles/active-directory/develop/access-tokens.md)
    :::column-end:::
    :::column:::
        **Microsoft-identitásplatform**

        - [Közönség](../articles/active-directory/develop/v2-supported-account-types.md)
        - [Alkalmazások és szolgáltatásnevek](../articles/active-directory/develop/app-objects-and-service-principals.md)
        - [Engedélyek és jóváhagyás](../articles/active-directory/develop/v2-permissions-and-consent.md)
    :::column-end:::
:::row-end:::
