...
description: Tekintse meg, hogyan állíthat be automatizált folyamatot az IoT-eszközök üzembe Azure Digital Twins a Device Provisioning Service (DPS) használatával.
author: baanders
ms.author: baanders
ms.date: 3/21/2021
ms.topic: how-to
...
Az ebben a cikkben ismertetett megoldással automatizálhatja **** a IoT Hub-eszközök Azure Digital Twins a Device Provisioning Service használatával. **** 

A kiépítési és  a kivezetési fázisokkal kapcsolatos további információkért, valamint az összes vállalati [  ](../iot-hub/iot-hub-device-management-overview.md#device-lifecycle) IoT-projekt általános eszközkezelési szakaszainak jobb érthetőségért tekintse meg az IoT Hub eszközkezelési dokumentációjának Eszköz életciklusa című szakaszát. 

## <a name="prerequisites"></a>Előfeltételek

A kiépítés beállítása előtt a következőket kell beállítania:
* egy **Azure Digital Twins példány.** Kövesse az [*Útmutató:*](how-to-set-up-instance-portal.md) Példány és hitelesítés beállítása egy Azure Digital Twins-példány létrehozásához útmutatását. Gyűjtse össze a példány **_gazdagépnevét_** a Azure Portal ([utasítások).](how-to-set-up-instance-portal.md#verify-success-and-collect-important-values)
* egy **IoT Hub.** Útmutatásért lásd *a* IoT Hub gyorsútmutató IoT Hub [szakaszát.](../iot-hub/quickstart-send-telemetry-cli.md)
* egy [**Azure-függvény,**](../azure-functions/functions-overview.md) amely frissíti a digitális ikeradatokat a IoT Hub alapján. Az Azure-függvény létrehozásához kövesse a [*How to: Ingest IoT hub data (Útmutató: IoT Hub-adatok*](how-to-ingest-iot-hub-data.md) bemenő adatainak be) útmutatását. Gyűjtse össze **_a függvény nevét_** a cikkben való használathoz.

Ez a minta egy eszközszimulátort **is használ,** amely magában foglalja a Device Provisioning Service használatával való kiépítést. Az eszközszimulátor itt található: [Azure Digital Twins és IoT Hub minta.](/samples/azure-samples/digital-twins-iothub-integration/adt-iothub-provision-sample/) A mintaprojektet a cím alatti Kód tallózása gombbal a mintahivatkozásra navigálva saját gépére is lekérte.  Ezzel a minta GitHub-adattárába fog vinni, amelyet letölthet a következőként: *. A* ZIP-fájlhoz válassza a **Kód gombot,** majd **a ZIP letöltése gombot.** 
...
A folyamat leírása:
1. Az eszköz kapcsolatba lép a DPS-végponttal, és azonosító információkat ad át az identitása bizonyítására.
2. A DPS úgy ellenőrzi az eszközidentitást, hogy érvényesíti a [](../azure-functions/functions-overview.md) regisztrációs azonosítót és a kulcsot a regisztrációs listán, és egy Azure-függvényt hív meg a lefoglaláshoz.
...
A következő szakaszok az automatikus kiépítési eszközfolyam beállításának lépéseit ismertetik.

### <a name="create-a-device-provisioning-service"></a>Device Provisioning Service létrehozása

Amikor új eszközt létesít a Device Provisioning Service-sel, az eszközhöz egy új ikereszköz is Azure Digital Twins a regisztrációs azonosítóval azonos névvel.
...
A következő Azure CLI-parancs létrehoz egy Device Provisioning Service-t. Meg kell adnia a Device Provisioning Service nevét, az erőforráscsoportot és a régiót. A Device Provisioning Service-t támogató régiókért tekintse meg a régiónként elérhető [*Azure-termékeket.*](https://azure.microsoft.com/global-infrastructure/services/?products=iot-hub)
...
:::image type="content" source="media/how-to-provision-using-device-provisioning-service/add-http-trigger-function-visual-studio.png" alt-text="Képernyőkép a Visual Studio nézetről, amely http-eseményindító típusú Azure-függvényt ad hozzá a függvényalkalmazás projektjéhez." lightbox="media/how-to-provision-using-device-provisioning-service/add-http-trigger-function-visual-studio.png":::

#### <a name="step-2-fill-in-function-code"></a>2. lépés: A függvénykód kitöltése

Adjon hozzá egy új NuGet-csomagot a projekthez: [Microsoft.Azure.Devices.Provisioning.Service.](https://www.nuget.org/packages/Microsoft.Azure.Devices.Provisioning.Service/) Előfordulhat, hogy a projekthez további csomagokat is hozzá kell adni, ha a kódban használt csomagok még nem részei a projektnek.

Az újonnan létrehozott függvénykódfájlban illessze be a következő kódot, nevezze át a függvényt *DpsAdtAllocationFunc.cs* névre, és mentse a fájlt.
...
Tegye közzé a projektet a *DpsAdtAllocationFunc.cs függvény* használatával az Azure-beli függvényalkalmazásban.
...
* **Válassza ki, hogyan szeretné hozzárendelni az eszközöket a központokhoz:** Egyéni (Azure-függvény használata).
...
:::image type="content" source="media/how-to-provision-using-device-provisioning-service/link-enrollment-group-to-iot-hub-and-function-app.png" alt-text="Képernyőkép a Központi regisztrációs csoport részletei ablakról, amely a Select Custom(Use Azure Function) (Egyéni(Azure-függvény használata) lehetőséget és az IoT Hub nevét tartalmazza a Select how you want to assign devices to hubs (Az eszközök központokhoz való hozzárendelésének mikéntjére) és a Select the IoT hubs this group can be assigned to (A csoporthoz hozzárendelhető IoT Hubok kiválasztása) szakaszban. Válassza ki az előfizetését és a függvényalkalmazást a legördülő menüből, és válassza a DpsAdtAllocationFunc lehetőséget." lightbox="media/how-to-provision-using-device-provisioning-service/link-enrollment-group-to-iot-hub-and-function-app.png":::

A regisztráció létrehozása után  a későbbiekben a regisztráció elsődleges kulcsát fogja használni a cikk eszközszimulátorának konfigurálhoz.

### <a name="set-up-the-device-simulator"></a>Az eszközszimulátor beállítása

Ez a minta egy eszközszimulátort használ, amely magában foglalja a Device Provisioning Service használatával való kiépítést. Az eszközszimulátor [](/samples/azure-samples/digital-twins-iothub-integration/adt-iothub-provision-sample/) az Előfeltételek szakaszban Azure Digital Twins IoT Hub [integrációs](#prerequisites) mintában található.

#### <a name="upload-the-model"></a>A modell feltöltése

Az eszközszimulátor egy termosztát típusú eszköz, amely a modellt használja a következő azonosítóval: `dtmi:contosocom:DigitalTwins:Thermostat;1` . Ezt a modellt fel kell töltenie a Azure Digital Twins, mielőtt létrehoz egy ikereszközt az eszközhöz.
...
A parancsablakban keresse meg a korábban kicsomagolt letöltött Azure Digital Twins és IoT Hub *integrációt,* majd lépjen az eszközszimulátor *könyvtárába.* Ezután telepítse a projekt függőségeit a következő paranccsal:
...
* PROVISIONING_IDSCOPE: Az érték lekért értékéhez lépjen az eszközátépítési szolgáltatáshoz a [Azure Portal](https://portal.azure.com/)területen, majd válassza az *Áttekintés* lehetőséget a menüben, és keresse meg a mező azonosítójának *hatóköre elemet.*

    :::image type="content" source="media/how-to-provision-using-device-provisioning-service/id-scope.png" alt-text="Képernyőkép az Azure Portal áttekintési oldalának képernyőképe az Azonosító hatóköre érték másolására." lightbox="media/how-to-provision-using-device-provisioning-service/id-scope.png":::
...
3. Egy Azure-függvény törli az eszköz ikereszközét a Azure Digital Twins.
...
Kövesse az Eseményközpont létrehozása [*rövid útmutatóban*](../event-hubs/event-hubs-create.md) leírt lépéseket. Nevezze el az eseményközpont *életciklusátevents néven.* Ezt az eseményközpontnevet a következő szakaszokban, az útvonal és IoT Hub Azure-függvény beállításakor fogja használni.
...
Ehhez:
1. Keresse meg az újonnan létrehozott eseményközpontot a  Azure Portal majd válassza a bal oldali menüben a Megosztott hozzáférési szabályzatok lehetőséget.
...
Ezután konfigurálja az előfeltételek szakaszban beállított [](#prerequisites) Azure-függvényalkalmazást, hogy az működjön az új eseményközpontbal. Ezt egy környezeti változónak az eseményközpont kapcsolati sztringje segítségével való beállításával fogja megtenni a függvényalkalmazásban.

1. Nyissa meg az előbb létrehozott szabályzatot, és másolja a **Kapcsolati sztring – elsődleges kulcs értékét.**

    :::image type="content" source="media/how-to-provision-using-device-provisioning-service/event-hub-sas-policy-connection-string.png" alt-text="Képernyőkép a Azure Portal-elsődleges kulcs másolásának feladatról." lightbox="media/how-to-provision-using-device-provisioning-service/event-hub-sas-policy-connection-string.png":::
...
Az előfeltételek szakaszban létrehozott függvényalkalmazás-projektben létrehoz egy új függvényt, amely egy meglévő eszközt fog kivezetni az IoT Hub életciklus-események használatával. [](#prerequisites)

Az életciklus-eseményekről további információt a nem [*IoT Hub eseményeket*](../iot-hub/iot-hub-devguide-messages-d2c.md#non-telemetry-events)tartalmazó IoT Hub. További információ az Azure Functions Event Hubs való használatával kapcsolatban: Azure Event Hubs [*eseményindító Azure Functions.*](../azure-functions/functions-bindings-event-hubs-trigger.md)
...
:::image type="content" source="media/how-to-provision-using-device-provisioning-service/create-event-hub-trigger-function.png" alt-text="Képernyőkép a Visual Studio ablakról, amely egy Event Hub Trigger típusú Azure-függvényt ad hozzá a függvényalkalmazás projektjéhez." lightbox="media/how-to-provision-using-device-provisioning-service/create-event-hub-trigger-function.png":::

#### <a name="step-2-fill-in-function-code"></a>2. lépés: A függvénykód kitöltése

Az újonnan létrehozott függvénykódfájlban illessze be a következő kódot, nevezze át a függvényt a névre, `DeleteDeviceInTwinFunc.cs` majd mentse a fájlt.
...
Most beállít egy útvonal-IoT Hub az eszközök életciklus-eseményeinek útválasztására. Ebben az esetben kifejezetten a által azonosított eszköz törlési eseményeit fogja `if (opType == "deleteDeviceIdentity")` figyelni. Ez aktiválja a digitális ikerelem törlését, és véglegesen törli az eszközt és annak digitális ikereszközét.
...
3. Válassza **a + Hozzáadás,** majd az **Eseményközpontok lehetőséget** egy eseményközpont-típusú végpont hozzáadásához.
    ...
    :::image type="content" source="media/how-to-provision-using-device-provisioning-service/event-hub-custom-endpoint.png" alt-text="Képernyőkép az Visual Studio új eseményközpont egyéni végpontjának hozzáadásáról." lightbox="media/how-to-provision-using-device-provisioning-service/event-hub-custom-endpoint.png":::
...
1. Lépjen az *Útvonalak lapra,* és válassza a **Hozzáadás lehetőséget** egy útvonal hozzáadásához.
    ...
    :::image type="content" source="media/how-to-provision-using-device-provisioning-service/add-message-route.png" alt-text="Képernyőkép az Visual Studio-ablakról, amely hozzáad egy útvonalat az események küldése számára." lightbox="media/how-to-provision-using-device-provisioning-service/add-message-route.png":::
...
2. A *megnyíló Útvonal hozzáadása* lapon válassza a következő értékeket:
   ...
   * **Útválasztási lekérdezés:** Írja be a `opType='deleteDeviceIdentity'` következőt: . Ez a lekérdezés úgy korlátozza az eszköz életciklus-eseményeit, hogy csak a törlési eseményeket küldjék el.

3. Kattintson a **Mentés** gombra.

    :::image type="content" source="media/how-to-provision-using-device-provisioning-service/lifecycle-route.png" alt-text="Képernyőkép az Azure Portal-ablakról, amely hozzáad egy útvonalat az életciklus-események küldése számára." lightbox="media/how-to-provision-using-device-provisioning-service/lifecycle-route.png":::

Miután végigment a folyamaton, minden úgy van beállítva, hogy végpontok között kiveszi az eszközöket.
...
2. A cikk első felében kiválasztott eszközregisztrációs azonosítóval fog [látni egy eszközt.](#auto-provision-device-using-device-provisioning-service) Másik lehetőségként bármely másik eszközt is törölhet, ha az ikereszköz az Azure Digital Twins-ban található, így ellenőrizheti, hogy az ikereszköz automatikusan törlődik-e az eszköz törlése után.
3. Jelölje ki az eszközt, majd válassza a **Törlés lehetőséget.**

:::image type="content" source="media/how-to-provision-using-device-provisioning-service/delete-device-twin.png" alt-text="Képernyőkép az Azure Portal ikereszköz IoT-eszközökről való törléséről." lightbox="media/how-to-provision-using-device-provisioning-service/delete-device-twin.png":::

A módosítások néhány percen belül megjelennek a Azure Digital Twins.

Az alábbi [parancssori Azure Digital Twins parancssori](how-to-use-cli.md) felületi paranccsal ellenőrizheti, hogy az eszköz ikereszköze Azure Digital Twins-példány törölve lett-e.
...
:::image type="content" source="media/how-to-provision-using-device-provisioning-service/show-retired-twin.png" alt-text="Az iker Parancsablak képernyőképe." lightbox="media/how-to-provision-using-device-provisioning-service/show-retired-twin.png":::

## <a name="clean-up-resources"></a>Az erőforrások eltávolítása

Ha már nincs szüksége a cikkben létrehozott erőforrásokra, az alábbi lépésekkel törölheti őket.

A Azure Cloud Shell vagy helyi Azure CLI használatával egy erőforráscsoport összes Azure-erőforrását törölheti az [az group delete paranccsal.](/cli/azure/group#az_group_delete) Ezzel eltávolítja az erőforráscsoportot; a Azure Digital Twins példány; az IoT Hub és a hub eszközregisztrációja; az Event Grid-témakör és a kapcsolódó előfizetések; az Event Hubs-névtér és mindkét Azure Functions, beleértve a társított erőforrásokat, például a tárolót.
...
Az eszközökhöz létrehozott digitális ikereszközök egyszintű hierarchiaként vannak tárolva az Azure Digital Twins-ban, de modellinformációk és a szervezet többszintű hierarchiája is bővítheti őket. Ha többet szeretne megtudni erről a fogalomról, olvassa el a következőt:

* [*Fogalmak: Digital Twins és az ikergráf*](concepts-twins-graph.md)
...
Írhat egyéni logikát, amely automatikusan biztosítja ezt az információt a modell és a gráfadatok használatával, amelyek már Azure Digital Twins. Az ikergráf információinak kezelésével, frissítésével és lekért értékével kapcsolatos további információkért tekintse meg a következőt:
