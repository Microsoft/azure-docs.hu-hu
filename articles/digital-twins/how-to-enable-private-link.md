---
title: Privát hozzáférés engedélyezése privát hivatkozással (előzetes verzió)
titleSuffix: Azure Digital Twins
description: 'Lásd: privát hozzáférés engedélyezése az Azure Digital Twins-megoldásokhoz privát hivatkozással'
author: baanders
ms.author: baanders
ms.date: 1/25/2021
ms.topic: how-to
...
ms.locfileid: "98948754"
---
# <a name="enable-private-access-with-private-link-preview"></a>Privát hozzáférés engedélyezése privát hivatkozással (előzetes verzió)
...
A példány kezdeti beállításának részeként bekapcsolhatja a privát hivatkozást egy Azure digitális Twins-példányhoz tartozó privát végponttal, vagy később is engedélyezheti egy olyan példányon, amely már létezik. 

Ezen létrehozási módszerek egyike ugyanazokat a konfigurációs beállításokat és a példányhoz tartozó végeredményt adja meg. Ez a szakasz azt ismerteti, hogyan végezhető el mindegyik a [Azure Portalban](https://portal.azure.com).
...
Ebben a szakaszban a [Azure Portal](https://portal.azure.com) használatával engedélyezheti a privát hivatkozásokat egy olyan Azure digitális Twins-példányon, amely éppen létre van hozva. Ez a szakasz a létrehozási folyamat hálózatkezelési lépésére koncentrál; az új Azure Digital Twins-példányok létrehozásának teljes áttekintését lásd: [*útmutató: példány és hitelesítés beállítása*](how-to-set-up-instance-portal.md).
...
Ebben a szakaszban a [Azure Portal](https://portal.azure.com) használatával engedélyezheti a privát hivatkozást egy olyan Azure digitális Twins-példányhoz, amely már létezik.
...
A hálózati jelző értékét a [Azure Portal](https://portal.azure.com), az [Azure CLI](/cli/azure/)vagy a [ARMClient parancssori eszköz](https://github.com/projectkudu/ARMClient)használatával frissítheti.

### <a name="option-1-using-the-azure-portal"></a>1. lehetőség: a Azure Portal használata
...
### <a name="option-2-using-the-azure-cli"></a>2. lehetőség: az Azure CLI használata

Az Azure CLI használatával ez a művelet az `az resource update` alább látható paranccsal hajtható végre. Az Azure Digital Twins-példány publicNetworkAccess tulajdonsága a vagy a értékre állítható be `disabled` `enabled` .

A jelző letiltásához használja a következő parancsot, és cserélje le a helyőrzőket a példány értékeire.
...
Itt látható egy képernyőkép arról, hogy a kimenet milyen módon fog kinézni.

:::image type="content" source="media/how-to-enable-private-link/network-flag-cli.png" alt-text="Képernyőkép a fenti Azure CLI-parancsot futtató terminál-ablakról. A kimenet a példány részleteit tartalmazza, beleértve a publicNetworkAccess nevű tulajdonságot, amelynek értéke letiltva." lightbox="media/how-to-enable-private-link/network-flag-cli.png":::

A jelző engedélyezéséhez használja a következő hasonló parancsot.
...
### <a name="option-3-usingarmclient"></a>3. lehetőség: a ARMClient használata  
