---
title: A Microsoft OPC közzétevő parancssori argumentumai
description: Ez a cikk áttekintést nyújt az OPC-közzétevő parancssori argumentumokról
author: jehona-m
ms.author: jemorina
...
ms.locfileid: "104787560"
---
# <a name="command-line-arguments"></a>Parancssori argumentumok

A következő több parancssori argumentumot ismertetünk, amelyek segítségével megadhatók az OPC-közzétevő globális beállításai.

## <a name="opc-publisher-command-line-arguments-for-version-25-and-below"></a>Az OPC-közzétevő parancssori argumentumai a 2,5-es vagy újabb verzióhoz

* Használat: opcpublisher.exe \<applicationname> [ \<iothubconnectionstring> ] [ \<options> ]
    
* ApplicationName: a használni kívánt OPC UA-alkalmazásnév az alkalmazás nevét is használja a közzétevő regisztrálásához a IoT Hub eszköz beállításjegyzékében.
    
* iothubconnectionstring: a IoT Hub tulajdonos ConnectionString, nem kötelező. Általában csak az alkalmazás első indításakor adhatja meg a IoTHub-tulajdonos ConnectionString karakterláncot. A rendszer titkosítja a kapcsolatok karakterláncát, és a platformok tanúsítványtárolójában tárolja.
Ezt követően a rendszer beolvassa a következő hívásokat, és újra felhasználja azt. Ha a ConnectionString-t minden egyes indításkor megadja, akkor a IoT Hub eszköz beállításjegyzékében az alkalmazáshoz létrehozott eszköz törlődik, és minden alkalommal újra létrejön.
    
Létezik néhány környezeti változó, amely az alkalmazás vezérlésére használható:
```
...
> [!NOTE] 
> Parancssori argumentumok felülbírálják a környezeti változók beállításait.
...
## <a name="opc-publisher-command-line-arguments-for-version-26-and-above"></a>OPC-közzétevő parancssori argumentumai a 2,6-es vagy újabb verzióhoz
```
      --pf, --publishfile=VALUE
                                       the filename to configure the nodes  to publish.
...
További források találhatók a GitHub-adattárakban:
