...
* Eszközfrissítéshez kiépítve [egy](device-update-agent-provisioning.md) IoT-eszköz (vagy szimulátor) a IoT Hub.
...
Most, hogy beállította az eszközfrissítést és kiépítte az eszközöket, szüksége lesz az eszközökre telepíteni kívánt frissítési fájl(ak)re.
...
4. Futtassa a következő parancsokat úgy, hogy lecseréli a mintaparaméter-értékeket egy importálási jegyzék létrehozásához, amely a frissítést leíró JSON-fájl:
    ...
    | updateVersion | Verziószám, amely megkülönbözteti ezt a frissítést a többitől, amelyek ugyanazokkal a Provider (Szolgáltató) és Name (Név) névvel is vannak. Nem egyezik az eszközön található egyes szoftverösszetevők verziójával (de igen, ha ön választja).
