...
Korlátozza a beállításjegyzékhez való hozzáférést a virtuális hálózat magánhálózati IP-címeinek a beállításjegyzék végpontjaihoz való hozzárendelésével és az [Azure Private Link.](../private-link/private-link-overview.md) A virtuális hálózaton lévő ügyfelek és a beállításjegyzék privát végpontjai közötti hálózati forgalom áthalad a virtuális hálózaton és egy privát kapcsolaton a Microsoft gerinchálózatán, így kiküszöbölve a nyilvános internetről való kitettséget. Private Link privát beállításjegyzék-hozzáférést is lehetővé tesz [](../expressroute/expressroute-introduction.MD) a helyszíni adatbázisból privát társviszony-létesítésen Azure ExpressRoute [VPN-átjárón keresztül.](../vpn-gateway/vpn-gateway-about-vpngateways.md)

A [beállításjegyzék privát végpontjainak DNS-beállításait](../private-link/private-endpoint-overview.md#dns-configuration) konfigurálhatja úgy, hogy a beállítások a beállításjegyzék számára lefoglalt privát IP-címre oldják fel a beállításokat. DNS-konfigurációval a hálózatban található ügyfelek és szolgáltatások továbbra is hozzáférhetnek a beállításjegyzékhez a beállításjegyzék teljes tartománynevével, például az *myregistry.azurecr.io.* 

Ez a szolgáltatás a **Prémium** tároló-beállításjegyzék szolgáltatási szinten érhető el. Jelenleg legfeljebb 10 privát végpontot lehet beállítani a beállításjegyzékhez. A beállításjegyzék szolgáltatási rétegeiről és korlátairól további információt a következő [Azure Container Registry tartalmaz:](container-registry-skus.md).
...
* Ha még nem rendelkezik tároló-beállításjegyzékpel, hozzon létre egyet (prémium szint szükséges), és importálja [a](container-registry-import-images.md) nyilvános rendszerképmintát, például a `mcr.microsoft.com/hello-world` Microsoft Container Registry. Regisztrációs adatbázis létrehozásához [Azure Portal][quickstart-portal] az [Azure CLI-t.][quickstart-cli]
* Ha a beállításjegyzékhez való hozzáférést egy másik Azure-előfizetésben található privát kapcsolat használatával konfigurálja, regisztrálnia kell az erőforrás-szolgáltatót az Azure Container Registry előfizetésben. Például:
...
Először futtassa [az az network private-endpoint show futtatását][az-network-private-endpoint-show] a hálózati adapter azonosítójának lekérdezéséhez:
...
Az alábbi [az network nic show][az-network-nic-show] parancsok a tároló-beállításjegyzék és a tárolójegyzék adatvégpontjának privát IP-címeit kapják meg:
...
> [!NOTE]
> Ha a beállításjegyzék [georeplikált,](container-registry-geo-replication.md)hozzon létre további DNS-rekordokat minden replika adatvégpontjának IP-címéhez.
...
A privát végpont létrehozása után a privát zónában lévő DNS-beállítások a portál **Privát** végpontok lapján jelennek meg:
...
Ha le szeretné tiltani a nyilvános hozzáférést az Azure CLI használatával, futtassa [az az acr update][az-acr-update] futtatását, és állítsa a következőre: `--public-network-enabled` `false` . 
...
Ellenőrizze, hogy a privát végpont alhálózatán belüli erőforrások magánhálózati IP-címen csatlakoznak-e a regisztrációs adatbázishoz, és hogy a megfelelő privát DNS-zónaintegrációval rendelkezik-e.

A privát kapcsolat kapcsolatának érvényesítéséhez SSH-kapcsolatot kell létesítenie a virtuális hálózaton beállított virtuális géppel.

Futtatassa az olyan segédprogramokat, mint a vagy a a beállításjegyzék `nslookup` `dig` IP-címét a privát kapcsolaton keresztül. Például:
...
Azt is ellenőrizze, hogy végrehajthat-e beállításjegyzék-műveleteket az alhálózaton lévő virtuális gépről. Létesítsen SSH-kapcsolatot a virtuális géppel, majd az [az acr login futtatásával][az-acr-login] jelentkezzen be a regisztrációs adatbázisba. A virtuálisgép-konfigurációtól függően előfordulhat, hogy a következő parancsokat előtaggal kell előtagként `sudo` használnia.

```bash
az acr login --name $REGISTRY_NAME
```

Hajtsa végre a beállításjegyzékbeli műveleteket, `docker pull` például egy rendszerképminta lekérte a beállításjegyzékből. Cserélje le a helyére a beállításjegyzéknek megfelelő rendszerképet és címkét, előtagként a beállításjegyzék bejelentkezési kiszolgálójának nevét `hello-world:v1` (csak kisbetűvel):
...
Kezelheti egy beállításjegyzék privát végpontkapcsolatát a Azure Portal vagy az [az acr private-endpoint-connection][az-acr-private-endpoint-connection] parancscsoport parancsai segítségével. A műveletek közé tartozik a regisztrációs adatbázis privát végpontkapcsolatának jóváhagyása, törlése, listába való felsorolása, elutasítása vagy megjelenítése.
...
Amikor a cikkben található lépésekkel privát végponti kapcsolatot hoz létre, a beállításjegyzék automatikusan fogadja a beállításjegyzékben Azure RBAC-engedélyekkel rendelkező ügyfelektől és szolgáltatásoktól származó kapcsolatokat. Beállíthatja úgy a végpontot, hogy manuálisan jóváhagyást igényel a kapcsolatokhoz. További információ a privát végponti kapcsolatok jóváhagyásáról és elutasításáról: [Privát végponti kapcsolat kezelése.](../private-link/manage-private-endpoint.md)

> [!IMPORTANT]
> Jelenleg, ha töröl egy privát végpontot egy beállításjegyzékből, előfordulhat, hogy törölnie kell a virtuális hálózat privát zónára mutató hivatkozását is. Ha a hivatkozás nincs törölve, a következő hibához hasonló hibaüzenet jelenhet meg: `unresolvable host` .

## <a name="dns-configuration-options"></a>DNS-konfigurációs beállítások

Ebben a példában a privát végpont egy alapszintű virtuális hálózathoz társított privát DNS-zónával integrálható. Ez a beállítás közvetlenül az Azure által biztosított DNS-szolgáltatást használja a regisztrációs adatbázis nyilvános teljes tartománynevének a virtuális hálózatban található magánhálózati IP-címekre való feloldásához. 
...
A beállításjegyzék nyilvános teljes tartományának magánhálózati IP-címre való feloldásához ezekben a forgatókönyvekben konfigurálnia kell egy kiszolgálószintű továbbító szolgáltatást az Azure DNS szolgáltatáshoz (168.63.129.16). A pontos konfigurációs beállítások és lépések a meglévő hálózatoktól és a DNS-től függenek. Példákért lásd: [Az Azure privát végpont DNS-konfigurációja.](../private-link/private-endpoint-dns.md)

> [!IMPORTANT]
> Ha a magas rendelkezésre állás érdekében több régióban hozott létre privát végpontokat, javasoljuk, hogy minden régióban használjon külön erőforráscsoportot, és helyezze el benne a virtuális hálózatot és a társított privát DNS-zónát. Ez a konfiguráció azt is megakadályozza, hogy a DNS feloldása kiszámíthatatlan módon, ugyanazon a privát DNS-zónán osztozik.
...
* Az Azure CLI használatával futtassa az [az network nic show][az-network-nic-show] parancsot. Például a parancsokról lásd [a cikk korábbi, Végpont IP-konfigurációjának](#get-endpoint-ip-configuration)le szolgáltatása.

* A portálon lépjen a privát végpontra, és válassza a **DNS-konfiguráció lehetőséget.**

A DNS-rekordok létrehozása után győződjön meg arról, hogy a beállításjegyzék teljes tartományneveinek feloldása megfelelő a megfelelő magánhálózati IP-címekre.
