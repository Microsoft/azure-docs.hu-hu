...
description: 'Rövid útmutató: Konzolalkalmazás létrehozása a .NET V4 SDK használatával az SQL API Azure Cosmos DB erőforrásainak kezeléséhez.'
author: anfeldma-ms
ms.author: anfeldma
...
ms.locfileid: "107873846"
---
# <a name="quickstart-build-a-console-app-using-the-net-v4-sdk-preview-to-manage-azure-cosmos-db-sql-api-account-resources"></a>Rövid útmutató: Konzolalkalmazás létrehozása a .NET V4 SDK (előzetes verzió) használatával az SQL API Azure Cosmos DB erőforrásainak kezeléséhez.
...
A .NET-hez Azure Cosmos DB SQL API ügyféloldali kódtárának első lépések. A dokumentum lépéseit követve telepítheti a .NET V4-es (Azure.Cosmos-) csomagot, létrehozhat egy alkalmazást, és kipróbálhatja az alapszintű CRUD-műveletekhez szükséges példakódot a Azure Cosmos DB.

> [!IMPORTANT]
> A .NET V4 SDK Azure Cosmos DB jelenleg nyilvános előzetes verzióban érhető el.
> Erre az előzetes verzióra nem vonatkozik szolgáltatói szerződés, és a használata nem javasolt éles számítási feladatok esetén. Előfordulhat, hogy néhány funkció nem támogatott, vagy korlátozott képességekkel rendelkezik.
> További információ: [Kiegészítő használati feltételek a Microsoft Azure előzetes verziójú termékeihez](https://azure.microsoft.com/support/legal/preview-supplemental-terms/).

Azure Cosmos DB a Microsofté™ gyors NoSQL-adatbázisa, nyílt API-kkel, bármilyen mérethez. Az adatbázis-Azure Cosmos DB kulcs-/érték-, dokumentum- és gráfadatbázisok gyors létrehozásához és lekérdezéséhez használhatja. Használja a Azure Cosmos DB SQL API .NET-hez való ügyféloldali kódtárát a következőre:
...
[Kódtár forráskódja](https://github.com/Azure/azure-cosmos-dotnet-v3/tree/v4)  |  [Csomag (NuGet)](https://www.nuget.org/packages/Azure.Cosmos)

## <a name="prerequisites"></a>Előfeltételek

* Azure-előfizetés [– hozzon létre](https://azure.microsoft.com/free/) egyet ingyenesen, vagy [kipróbálhatja Azure Cosmos DB](https://azure.microsoft.com/try/cosmosdb/) ingyenes előfizetés, ingyenes és kötelezettségvállalások nélkül. 
...
Ez a szakasz végigvezeti egy Azure Cosmos-fiók létrehozásán és egy olyan projekt beállításán, amely a .NET-hez Azure Cosmos DB SQL API ügyféloldali kódtárát használja az erőforrások kezeléséhez. A cikkben leírt példakód létrehoz egy adatbázist és egy családtagot (minden családtag egy elem) az `FamilyDatabase` adatbázisban. Minden családtag rendelkezik olyan tulajdonságokkal, mint a `Id, FamilyName, FirstName, LastName, Parents, Children, Address,` . A `LastName` tároló partíciókulcsa a tulajdonság. 
...
1. Nyissa meg **a Kulcsok panelt,** és másolja ki a fiók **URI-ját** és **ELSŐDLEGES** KULCSát. A következő lépésben hozzáadja az URI és a kulcsok értékeit egy környezeti változóhoz.
...
* CreateDatabaseIfNotExistsAsync – Ez a metódus (ha még nem létezik) vagy (ha már létezik) adatbázis-erőforrást hoz létre aszinkron műveletként. 
* CreateContainerIfNotExistsAsync – Ez a metódus aszinkron műveletként hoz létre (ha még nem létezik) vagy lekért (ha már létezik). A válasz állapotkódját ellenőrizve megállapíthatja, hogy a tároló újonnan lett létrehozva (201) vagy egy meglévő tároló lett visszaadva (200). 
...
Töröljön egy családelemet a `DeleteFamilyItemAsync` metódus hozzáadásával az alábbi kóddal.
...
Ebben a rövid útmutatóban megtanulta, hogyan hozhat létre Azure Cosmos-fiókot, hogyan hozhat létre adatbázist és tárolót egy .NET Core-alkalmazással. Most már importálhat további adatokat az Azure Cosmos-fiókjába a következő cikk utasításait követve. 
