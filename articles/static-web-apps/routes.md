...
ms.locfileid: "107313640"
---
# <a name="routes-in-azure-static-web-apps-preview"></a>Útvonalak az Azure statikus Web Apps előzetes verziójában

> [!IMPORTANT]
> A fájl _routes.jsban_ definiált funkció már elavult, és jobban megvalósítható az Azure statikus Web Apps [konfigurációs fájljában](./configuration.md#routes).

Az Azure statikus Web Apps útválasztása meghatározza a háttér-útválasztási szabályokat és a statikus tartalom és<sup>az API-</sup>k engedélyezési viselkedését. A szabályok a fájlban lévő _staticwebapp.config.js_ szabályok tömbje vannak meghatározva.

- A fájl _staticwebapp.config.jsjának_ léteznie kell az alkalmazás Build-összetevő mappájának gyökerében.
...
- A szerepkörök a fájl _staticwebapp.config.jsban_ vannak definiálva, és a felhasználók [meghívón](authentication-authorization.md)keresztül vannak társítva a szerepkörökhöz.
...
A fájl _staticwebapp.config.jsjának_ léteznie kell az alkalmazás Build-összetevő mappájának gyökerében. Ha a webalkalmazás olyan Build lépést tartalmaz, amely egy adott mappából a Build-összetevő mappájába másolt létrehozott fájlokat, akkor a fájl _staticwebapp.config.jsának_ léteznie kell az adott mappában.

A következő táblázat felsorolja a megfelelő helyet, amellyel a _staticwebapp.config.jsa_ fájl számos előtér-keretrendszerhez és-könyvtárhoz elhelyezhető.
...
Az útvonalak a (z) _staticwebapp.config.js_ fájlon vannak definiálva az útválasztási szabályok tömbje a `routes` tulajdonságon. Minden szabály egy útvonal mintából áll, valamint egy vagy több választható szabály-tulajdonsággal. A használati példákat lásd a [példaként szolgáló útválasztási fájlban](#example-route-file) .
...
Előfordulhat, hogy a felhasználók számos különböző szituációba ütköznek, ami hibát okozhat. A tömb használatával egyéni felhasználói élményt adhat meg a `platformErrorOverrides` hibákra reagálva. Tekintse át a [példaként szolgáló útválasztási fájlt](#example-route-file) a tömb elhelyezéséhez a fájl _staticwebapp.config.js_ .
...
- A _staticwebapp.config.jsban_ definiált fejlécek csak statikus tartalomra vonatkoznak. A függvény kódjában testreszabhatja az API-végpontok válaszának fejléceit.
...
Az alábbi példa bemutatja, hogyan hozhat létre a statikus tartalomra és API-kra vonatkozó útválasztási szabályokat a fájl _staticwebapp.config.js_ . Egyes útvonalak a [ _/.auth_ rendszermappáját](authentication-authorization.md) használják, amely hozzáfér a hitelesítéssel kapcsolatos végpontokhoz.
...
| _/customers/contoso_               | A rendszergazdák vagy _customers_contoso_ szerepkörökhöz tartozó hitelesített felhasználók a _/Customers/contoso/index.html_ <sup>2</sup>. fájlt _kapják_ meg. A _rendszergazdák_ vagy _customers_contoso_ szerepkörökben nem szereplő hitelesített felhasználók 401 hibát szolgáltatnak. Nem hitelesített felhasználók átirányítva a _belépési_ értékre. |
...
- A fájl _staticwebapp.config.jsa_ nem lehet nagyobb, mint 100 kb
- A fájl _staticwebapp.config.js_ legfeljebb 50 különböző szerepkört támogat
