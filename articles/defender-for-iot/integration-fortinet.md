...
## <a name="the-defender-for-iot-platform"></a>A Defender for IoT platform
...
A szervezetek programozható logikai vezérlőket (PLC-ket) használnak a fizikai folyamatok, például a gyárak, a szélerőművek és a nukleáris erőművek Centrifugálja kezelésére.
...
A FortiGate tűzfalon a IoT riasztások alapján automatikusan beállíthatja a blokkoló házirendeket.
...
1. Mentse az API-kulcsot.

    :::image type="content" source="media/integration-fortinet/api-key.png" alt-text="A mobiltelefon leírása automatikusan új API-kulcsot hoz létre":::

## <a name="block-suspected-traffic-using-the-fortigate-firewall"></a>A feltételezett forgalom blokkolása a FortiGate tűzfal használatával

1. A bal oldali ablaktáblán válassza a **továbbítás** lehetőséget.

    [:::image type="content" source="media/integration-fortinet/forwarding-view.png" alt-text="Az érzékelő továbbító ablakának beállítása":::](media/integration-fortinet/forwarding-view.png#lightbox)

1. Válassza a **továbbítási szabályok létrehozása** és a szabály paramétereinek megadása lehetőséget.

    :::image type="content" source="media/integration-fortinet/new-rule.png" alt-text="A továbbítási szabály létrehozása ablak képernyőképe":::
    ...
    | **Protokollok** | Alapértelmezés szerint az összes protokoll ki van választva.<br><br>Egy adott protokoll kiválasztásához válassza az **adott** lehetőséget, majd válassza ki azt a protokollt, amelyre ez a szabály vonatkozik. |
    | **Végrehajtók** | Alapértelmezés szerint a rendszer minden biztonsági motort érint.<br><br>Egy adott biztonsági motor kiválasztásához, amelyhez ez a szabály vonatkozik, válassza az **adott** lehetőséget, majd válassza ki a motort. |
    | **Rendszerértesítések** | Az érzékelő *online és offline* állapotának továbbítása. Ez a lehetőség csak akkor érhető el, ha bejelentkezett a helyszíni felügyeleti konzolba. |

1. Ha azt szeretné, hogy a Defender IoT küldjön a FortiGate, válassza a **művelet** , majd **a Küldés a FortiGate** lehetőséget.

    :::image type="content" source="media/integration-fortinet/fortigate.png" alt-text="A továbbítási szabály létrehozása ablakban válassza a Küldés a FortiGate":::

1. A FortiGate-továbbítási szabály konfigurálása:

    :::image type="content" source="media/integration-fortinet/configure.png" alt-text="A továbbítási szabály létrehozása ablak konfigurálása":::

1. A **műveletek** ablaktáblán állítsa be a következő paramétereket:

    | Paraméter | Leírás |
    |--|--|
    | Gazdagép | A FortiGate-kiszolgáló IP-címének típusa. |
    | Port | A FortiGate-kiszolgáló portjának típusa. |
    | Felhasználónév | A FortiGate-kiszolgáló Felhasználónév típusa. |
    | API-kulcs | Adja meg a FortiGate-ben létrehozott API-kulcsot. |
    | Bejövő felület| A blokkolás végrehajtásának meghatározása:<br /><br />**IP-cím szerint**: mindig blokkolja a zárolási házirendeket az IP-címeken alapuló panorámában.<br /><br />**FQDN vagy IP-cím szerint**: blokkolja a kikapcsolási házirendeket a teljes tartománynév alapján, ha létezik, máskülönben IP-cím.| 
    | Kimenő csatoló |Adja meg a szabályzat értesítő e-mail-címét. <br /><br /> **Megjegyzés**: Győződjön meg róla, hogy konfigurált egy levelezési kiszolgálót a XSense-ben. Ha nincs megadva e-mail cím, a XSense nem küld értesítő e-mailt.| 
    |Konfigurálás| A következő beállítások beállításával engedélyezheti a gyanús források blokkolását a FortiGate tűzfalon: <br /><br />**Tiltott kódok blokkolása**: protokollok megsértése – nem engedélyezett mezőérték – az ICS protokoll specifikációjának megsértve (lehetséges biztonsági rés)<br /><br />**Jogosulatlan PLC programozási/belső vezérlőprogram frissítéseinek letiltása**: jogosulatlan PLC-változások<br /><br />Nem **engedélyezett PLC leállítása**: PLC leállítása (leállás)<br /><br />**Kártevők elleni riasztások letiltása**: az ipari kártevő-kísérletek blokkolása (TRITON, NotPetya stb.). Választhatja az **automatikus blokkolás** lehetőségét. Ebben az esetben a blokkolás automatikusan és azonnal elindul.<br /><br />*Jogosulatlan vizsgálat letiltása*: jogosulatlan vizsgálat (lehetséges felderítés)
