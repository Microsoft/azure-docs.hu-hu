...
description: A rendezett fürtözött oszlopcentrikus indexnek a dedikált SQL-készletekben való lekérdezési teljesítményének javítása érdekében figyelembe kell vennie a javaslatokat és szempontokat.
...
Amikor a felhasználók egy dedikált SQL-készletben lévő oszlopcentrikus táblát lekérdeznek, az optimalizáló ellenőrzi az egyes szegmensek minimális és maximális értékeit.  A lekérdezési predikátumon kívüli szegmenseket a rendszer nem olvassa be a lemezről a memóriába.  A lekérdezések gyorsabb teljesítményt eredményeznek, ha az olvasni kívánt szegmensek száma és a teljes méretük kicsi.   

## <a name="ordered-vs-non-ordered-clustered-columnstore-index"></a>Rendezett és nem rendezett fürtözött oszlopcentrikus index

Alapértelmezés szerint minden indexbeállítás nélkül létrehozott táblához egy belső összetevő (indexszerkesztő) létrehoz egy nem rendezett fürtözött oszlopcentrikus indexet (CCI).  Az egyes oszlopokban lévő adatok külön CCI sorcsoport-szegmensbe tömörítve adatokat tartalmaznak.  Az egyes szegmensek értéktartományában metaadatok vannak, így a lekérdezési predikátumon kívüli szegmensek nem olvashatók be a lemezről a lekérdezés végrehajtása során.  A CCI a legmagasabb szintű adattömörítést kínálja, és csökkenti az olvasni kívánt szegmensek méretét, így a lekérdezések gyorsabban futnak. Mivel azonban az indexszerkesztő nem rendezi az adatokat, mielőtt szegmensekre tömöríti őket, átfedésben lévő értéktartományokkal is előfordulhatnak szegmensek, így a lekérdezések több szegmenst olvasnak be a lemezről, és hosszabb ideig tart a befejezése.  

Rendezett CCI létrehozásakor a dedikált SQL-készletmotor a rendelési kulcs(ak) szerint rendezi a memóriában lévő adatokat, mielőtt az indexszerkesztő indexszegmensekbe tömöríti azokat.  A rendezett adatok esetén a szegmensek átfedődése csökkenti a szegmensek hatékonyabb megszüntetését és ezáltal a teljesítményt, mivel a lemezről beolvasható szegmensek száma kisebb.  Ha minden adat egyszerre rendezhető a memóriában, elkerülhető a szegmensek átfedése.  Az adattárház nagy méretű táblái miatt ez a forgatókönyv nem gyakran fordul elő.  
...
Az ezekkel a mintákkal kapcsolatos lekérdezések általában gyorsabbak a rendezett CCI-k használatával.  
...
Az átfedésben lévő szegmensek száma a rendezni kívánt adatok méretétől, a rendelkezésre álló memóriától és a maximális párhuzamosság mértékétől (MAXDOP) függ a rendezett CCI létrehozása során. Az alábbi lehetőségek a szegmensek átfedésének csökkentésére kínálnak lehetőséget a rendezett CCI létrehozásakor.

- Használja az xlargerc erőforrásosztályt egy magasabb DWU-ban, hogy több memória rendezési lehetőséget engedélyezzen, mielőtt az indexkészítő szegmensekbe tömöríti az adatokat.  Az indexszegmensben az adatok fizikai helye nem módosítható.  A szegmensen belül és a szegmensek között nem rendezési adatok vannak.  

- Rendezett CCI létrehozása MAXDOP = 1 értékekkel.  A rendezett CCI-létrehozáshoz használt szálak az adatok egy részkészletét használják, és helyileg rendezik őket.  Az adatok között nincs globális rendezés a különböző szálak szerint rendezve.  A párhuzamos szálak használata csökkentheti egy rendezett CCI létrehozásához szükséges időt, de több átfedő szegmenst hoz létre, mint egyetlen szál használata.  A MAXDOP beállítás jelenleg csak rendezett CCI-tábla az AS SELECT paranccsal CREATE TABLE létrehozásakor támogatott.  Rendezett CCI létrehozása CREATE INDEX vagy CREATE TABLE parancsokkal nem támogatja a MAXDOP beállítást. Példa:
...
- Rendezheti előre az adatokat a rendezési kulcs(ak) alapján, mielőtt betölti őket a táblákba.

Lássunk egy példát egy olyan rendezett CCI-táblaeloszlásra, amely a fenti javaslatok szerint nulla szegmenst fed át. A rendezett CCI-tábla egy DWU1000c adatbázisban jön létre a CTAS használatával egy 20 GB-os halomtáblából a MAXDOP 1 és az xlargerc használatával.  A CCI egy BIGINT oszlopra van rendelve, ismétlődések nélkül.  

![Segment_No_Overlapping](./media/performance-tuning-ordered-cci/perfect-sorting-example.png)

## <a name="create-ordered-cci-on-large-tables"></a>Rendezett CCI létrehozása nagyméretű táblákon

A rendezett CCI létrehozása offline művelet.  Partíciót nem használó táblák esetén az adatok nem lesznek elérhetők a felhasználók számára, amíg a rendezett CCI-létrehozási folyamat be nem fejeződik.   Particionált táblák esetén, mivel a motor partíciók szerint hozza létre a rendezett CCI-partíciót, a felhasználók továbbra is hozzáférhetnek az adatokhoz olyan partíciókban, ahol a rendezett CCI létrehozása még nincs folyamatban.   Ezzel a beállítással minimalizálhatja az állásidőt a CCI nagy táblákon való rendezett létrehozása során: 
...
6.    Miután az összes partíciót átkapcsolta Table_A-ről Table_B- és újraépítette, Table_A, és nevezze át a Table_B-Table_A. 
...
>
> A CCI rendezett karbantartásával kapcsolatos további információkért lásd: [Fürtözött oszlopcentrikus indexek optimalizálása.](sql-data-warehouse-tables-index.md#optimizing-clustered-columnstore-indexes)
