---
title: Az Azure kommunikációs szolgáltatások csapatának beágyazása Android-alkalmazásokhoz
description: Ebből az áttekintésből megtudhatja, hogyan használhatja az Azure kommunikációs szolgáltatások csapatának beágyazási könyvtárát az Android rendszerhez.
author: palatter
ms.author: palatter
ms.date: 24/02/2021
ms.topic: conceptual
...
Adja hozzá a `MeetingEventListener` osztályhoz.
...
Állítsa be a `MeetingEventListener` -ben `this` .
...
A `onCallStateChanged` és `onRemoteParticipantCountChanged` metódusok implementálása.
...
Adja hozzá a `MeetingIdentityProvider` osztályhoz.
...
Állítsa be a `MeetingIdentityProvider` -ben `this` .
...
Rendelje hozzá `userMri` a megfelelő avatart.
