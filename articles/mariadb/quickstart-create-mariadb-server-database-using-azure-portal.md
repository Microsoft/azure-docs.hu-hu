...
ms.locfileid: "107531356"
---
# <a name="quickstart-create-an-azure-database-for-mariadb-server-by-using-the-azure-portal"></a>Rövid útmutató: Azure Database for MariaDB-kiszolgáló létrehozása a Azure Portal
...
2. Válassza **az Adatbázisok**  >  **Azure Database for MariaDB** lehetőséget. A keresőmezőbe be is írhatja a **MariaDB-t,** hogy megtalálja a szolgáltatást.
...
3. Adja meg vagy válassza ki a következő kiszolgálóadatokat:
   ...
   > [!NOTE]
   > Akkor érdemes az Alapszintű tarifacsomagot használni, ha a kis számítási és I/O-kapacitás megfelelő a számítási feladathoz. Vegye figyelembe, hogy az Alapszintű tarifacsomagban létrehozott kiszolgálók később nem skálázhatóak általános célú memóriaoptimalált kiszolgálókra. További [információért tekintse meg a](https://azure.microsoft.com/pricing/details/mariadb/) díjszabási oldalt.
...
4. A **kiszolgáló üzembehelyezéséhez válassza** az Áttekintés + létrehozás lehetőséget. Az üzembe helyezés akár 20 percet is igénybe vehet.
...
A példánkban a kiszolgálónév **mydemoserver.mariadb.database.azure.com** a kiszolgáló-rendszergazdai bejelentkezési név **myadmin \@ mydemoserver.**
...
2. Új kapcsolat létrehozásához kattintson a **+** **MySQL-kapcsolatok** fejléc melletti plusz ikonra ( ).
...
3. A **Setup New Connection** (Új kapcsolat beállítása) párbeszédpanelen adja meg a kiszolgáló kapcsolatadatait a **Parameters** (Paraméterek) lapon. A helyőrző értékek példaként szerepelnek. A **Hostname** (Gazdagépnév), **Username** (Felhasználónév) és **Password** (Jelszó) értékei helyére írja a saját értékeit.
   ...
   ![Új kapcsolat beállítása](./media/quickstart-create-mariadb-server-database-using-azure-portal/setup-new-connection.png)
    ...
    Felhasználónév |  *kiszolgáló-rendszergazdai bejelentkezési név* | Azok a kiszolgáló-rendszergazdai bejelentkezési adatok, amelyeket korábban az Azure Database for MariaDB-kiszolgáló létrehozásakor használt. A példában a felhasználónév **myadmin \@ mydemoserver.** Ha nem emlékszik a felhasználónevére, a kapcsolati adatok lekéréséhez kövesse a cikk korábbi szakaszában ismertetett lépéseket. A formátum a *következő: felhasználónév \@ kiszolgálónév.*
