...
### <a name="cmk-recommendations-moved-to-best-practices-security-control"></a>A CMK-javaslatok átkerültek az ajánlott eljárások biztonsági vezérlése alá
...
- Az SQL-kiszolgálóknak ügyfél által felügyelt kulcsokat kell használniuk az adatok titkosításához
...
### <a name="11-azure-defender-alerts-deprecated"></a>11 Azure Defender riasztások elavultak

Az Azure Defender felsorolt riasztások elavultak.
...
- [Azure Firewall integrált Security Center](#azure-firewall-management-integrated-into-security-center)
- [Az SQL sebezhetőségi felmérése mostantól tartalmazza a "Szabály letiltása" élményt (előzetes verzió)](#sql-vulnerability-assessment-now-includes-the-disable-rule-experience-preview)
...
- [A jogszabályi megfelelés irányítópultja mostantól tartalmazza az Azure auditjelentéseit (előzetes verzió)](#regulatory-compliance-dashboard-now-includes-azure-audit-reports-preview)
...
További információ: [Egyes eredmények letiltása.](defender-for-sql-on-machines-vulnerability-assessment.md#disable-specific-findings-preview)
...
### <a name="recommendation-data-can-be-viewed-in-azure-resource-graph-with-explore-in-arg"></a>A javaslati adatok a "Explore in ARG Azure Resource Graph" (Feltárás az ARG-ban)

A javaslat részleteit tartalmazó oldalakon most már szerepel az "Explore in ARG" (Felfedezés az ARG-ban) eszköztárgomb. Ezzel a gombbal megnyithatja a Azure Resource Graph, és feltárhatja, exportálhatja és megoszthatja a javaslat adatait.

Azure Resource Graph (ARG) azonnali hozzáférést biztosít az erőforrás-információkhoz a felhőalapú környezetekben robusztus szűrési, csoportosítási és rendezési képességekkel. Ez egy gyors és hatékony módszer az információk programozott módon vagy a felhőben történő lekérdezésére az Azure-előfizetések Azure Portal.
...
- Hozzárendeléskor a kényszerítési funkció továbbra is engedélyezve lesz.
- Most már testreszabhatja ezeket a szabályzatokat, és frissítheti a paramétereket, még akkor is, ha már üzembe vannak telepítve. Ha például egy felhasználó egy másik értékelési kulcsot szeretne hozzáadni, vagy szerkeszteni szeretne egy meglévő értékelési kulcsot, azt is meg tudja tenni.

Munkafolyamat-automatizálási [sablonok – első lépések.](https://github.com/Azure/Azure-Security-Center/tree/master/Workflow%20automation)

További információ az [eseményindítókra adott válaszok automatizálásával Security Center kapcsolatban.](workflow-automation.md)
...
Két javaslathoz az adatokat a rendszer egyidejűleg közvetlenül az Azure-tevékenységnaplóba írja. Ezzel a módosítással a Security Center leállítja az adatok írását az örökölt biztonsági javaslatokhoz közvetlenül a tevékenységnaplóba. Ehelyett az adatokat a Azure Advisor exportáljuk, mint a többi javaslat esetében.
...
1. A címkéket (például Gyorsjavítás és Előzetes **verzió)** lecserélő **ikonok.**
1. Egy új oszlop, amely az egyes javaslatokhoz kapcsolódó [Szabályzat kezdeményezést](security-policy-concept.md) mutatja – akkor látható, ha a "Csoportosítás vezérlők szerint" le van tiltva.

:::image type="content" source="media/release-notes/recommendations-grid-enhancements.png" alt-text="A Azure Security Center javaslatok oldalának fejlesztései – 2021. március" lightbox="media/release-notes/recommendations-grid-enhancements.png":::

:::image type="content" source="media/release-notes/recommendations-grid-enhancements-initiatives.png" alt-text="Az Azure Security Center &quot;lapos&quot; listájának fejlesztései – 2021. március" lightbox="media/release-notes/recommendations-grid-enhancements-initiatives.png":::
...
- **Mintariasztás** létrehozása funkció – Mintariasztásokat hozhat létre az új riasztások felhasználói élményből: Mintariasztás [Azure Defender létrehozása.](security-center-alert-validation.md#generate-sample-azure-defender-alerts)
...
### <a name="kubernetes-workload-protection-recommendations-released-for-general-availability-ga"></a>A Kubernetes számítási feladatok védelmére vonatkozó javaslatok általánosan elérhetőek (GA)
...
### <a name="workflow-automations-can-be-triggered-by-changes-to-regulatory-compliance-assessments-in-preview"></a>A munkafolyamat-automatizálást a jogszabályi megfelelési értékelések változásai aktiválják (előzetes verzióban)
Hozzáadtunk egy harmadik adattípust a munkafolyamat-automatizálások eseményindítóihoz: a jogszabályi megfelelési értékelések változásait.
...
- [A biztonságirés-felmérést a rendszer általánosan elérhetőként (GA) ad ki a többfelhős és a felhőalapú számítógépeken.](#vulnerability-assessment-for-on-premise-and-multi-cloud-machines-is-released-for-general-availability-ga)
...
- [Az Azure Defender dns-App Service](#dangling-dns-protections-added-to-azure-defender-for-app-service)
...
Ha az Security Center megfelelőségi irányítópultját használja, a teljesítményteszt két példánya fog látni az átmeneti időszakban:
...
A meglévő javaslatokat ez nem befolyásolja, és a teljesítményteszt növekedésével a változások automatikusan megjelennek a Security Center. 
...
A biztonsági pontszám oldal mostantól az előfizetési szint mellett a felügyeleti csoportok összesített biztonsági pontszámait is megjeleníti. Most már láthatja a szervezetben található felügyeleti csoportok listáját és az egyes felügyeleti csoportok pontszámát.
...
- [A dangolásos DNS-bejegyzések](../security/fundamentals/subdomain-takeover.md) megakadályozása és az altartományok feladatátvételének elkerülése – További információ az altartományok átvételének fenyegetésről és az összeomó DNS-aspektusról
- [Bevezetés a Azure Defender-App Service](defender-for-app-service-introduction.md)
...
- Automatikus ügynökátépítés – Security Center a Azure Arc a Log Analytics-ügynök AWS-példányon való üzembe helyezéséhez
- Szabályzatkezelés
...
Bővítjük a kivétel képességet, hogy teljes javaslatokat tartalmazzanak. További lehetőségeket biztosít az előfizetések, felügyeleti csoportok vagy erőforrások Security Center által nyújtott biztonsági javaslatok finomhangolásához.

Előfordulhat, hogy egy erőforrás nem megfelelőként lesz listázva, ha tudja, hogy a problémát egy olyan külső eszköz oldotta meg, Security Center nem észlelte. Az is lehet, hogy egy javaslat olyan hatókörben fog jelenni, amelyről úgy érzi, hogy nem tartozik hozzá. Előfordulhat, hogy a javaslat nem megfelelő egy adott előfizetéshez. Vagy lehet, hogy a szervezete úgy döntött, hogy elfogadja az adott erőforrással vagy javaslattal kapcsolatos kockázatokat.
...
### <a name="not-applicable-resources-now-reported-as-compliant-in-azure-policy-assessments"></a>A "Nem alkalmazható" erőforrások mostantól "Megfelelőként" jelentve a Azure Policy értékelésekben

Korábban az adott javaslatra kiértékelt és  nem alkalmazhatónak talált erőforrások a Azure Policy "Nem megfelelő" szövegként jelentek meg. Egyetlen felhasználói művelet sem módosíthatja az állapotát "Megfelelő" állapotra. Ezzel a módosítással a rendszer "Megfelelőként" jelenti őket a jobb átláthatóság érdekében.
...
- **Azure Defender gépeken található** SQL-kiszolgálókhoz – kiterjeszti ugyanezeket a védelmet a hibrid, többfelhős és helyszíni környezetekben található SQL-kiszolgálókra is
...
A Security Center és funkcióinak bővítése során a biztonsági javaslatok listája havonta növekszik. Lásd például [a 29 előzetes verzióra vonatkozó javaslatot az Azure-biztonsági](#29-preview-recommendations-added-to-increase-coverage-of-azure-security-benchmark)teljesítményteszt lefedettségének növelése érdekében.
...
- **Válaszműveletek** – A javaslatok megtekintése a következő válaszbeállítások Security Center alapján: Gyorsjavítás, Megtagadás és Kényszerítés
    ...
    > - [Gyorsjavítás a szervizeléshez](security-center-remediate-recommendations.md#quick-fix-remediation)
    > - [Helytelen konfigurációk megelőzése Kényszerítés/Megtagadás javaslatokkal](prevent-misconfigurations.md)

:::image type="content" source="./media/release-notes/added-recommendations-filters.png" alt-text="Biztonsági vezérlők szerint csoportosított javaslatok" lightbox="./media/release-notes/added-recommendations-filters.png":::
...
### <a name="29-preview-recommendations-added-to-increase-coverage-of-azure-security-benchmark"></a>29 előzetes verzióra vonatkozó javaslat az Azure-biztonsági teljesítményteszt lefedettségének növeléséhez
...
### <a name="nist-sp-800-171-r2-added-to-security-centers-regulatory-compliance-dashboard"></a>NIST SP 800 171 R2 hozzáadva az Security Center megfelelőségi irányítópultjához

Az NIST SP 800-171 R2 szabvány mostantól beépített kezdeményezésként érhető el az Azure Security Center szabályozási megfelelőségi irányítópultjához. A vezérlők leképezéseit a [Details of the NIST SP 800-171 R2 Regulatory Compliance (NIST SP 800-171 R2 szabályozási](../governance/policy/samples/nist-sp-800-171-r2.md)megfelelőség) beépített kezdeményezés részleteiben ismertetjük. 

Ha a szabványt alkalmaznia kell az előfizetésére, és folyamatosan figyelnie kell a megfelelőségi állapotot, használja a Szabványkészlet testreszabása a jogszabályi megfelelőségi [irányítópulton útmutatót.](update-regulatory-compliance-packages.md)

:::image type="content" source="media/release-notes/nist-sp-800-171-r2-standard.png" alt-text="Az NIST SP 800 171 R2 szabvány az Security Center megfelelőségi irányítópultján":::
...
- a biztonsági pontszám nyomon követése az idő alatt dinamikus jelentésekkel
...
### <a name="policy-management-page-in-the-azure-portal-now-shows-status-of-default-policy-assignments"></a>A szabályzatkezelési oldal Azure Portal megjeleníti az alapértelmezett szabályzat-hozzárendelések állapotát
