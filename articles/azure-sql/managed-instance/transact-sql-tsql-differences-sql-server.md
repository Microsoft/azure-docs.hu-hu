...
- Az elosztott írható tranzakciók csak felügyelt példányok között lehetségesek. További információ: [Elosztott tranzakciók](../database/elastic-transactions-overview.md). Az MS DTC azonban nem támogatott.
...
- A [példányok közötti](../database/elastic-transactions-overview.md) írási tranzakciók csak felügyelt példányok esetén támogatottak.
...
A több példányban futó Service Broker-üzenetváltás csak az Azure SQL felügyelt példányai között támogatott:

- `CREATE ROUTE`: Nem használható `CREATE ROUTE` `ADDRESS` más `LOCAL` SQL felügyelt példányhoz tartozó vagy más DNS-névvel.
- `ALTER ROUTE`: Nem használható `ALTER ROUTE` `ADDRESS` más `LOCAL` SQL felügyelt példányhoz tartozó vagy más DNS-névvel.

A szállítás biztonsága támogatott, a párbeszédablakok biztonsága nem:
...
- `Extended stored procedures` nem támogatott, és ez a következőt tartalmazza: `sp_addextendedproc` és `sp_dropextendedproc` . Ez a funkció nem támogatott, mert a SQL Server elavult elérési úton van. További részletekért lásd a [kiterjesztett tárolt eljárások](/sql/relational-databases/extended-stored-procedures-programming/database-engine-extended-stored-procedures-programming)című témakört.
