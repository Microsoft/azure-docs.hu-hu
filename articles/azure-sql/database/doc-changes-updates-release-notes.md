...
ms.locfileid: "107600913"
---
# <a name="whats-new-in-azure-sql-database--sql-managed-instance"></a>Újdonságok a Azure SQL Database & SQL Managed Instance?
...
A dokumentáció Azure SQL Database és Azure SQL Managed Instance szakaszokra oszlik. Azt is frissítettük, hogyan hivatkozunk egy felügyelt példányra a felügyelt *Azure SQL Database-* és *Azure SQL Managed Instance.*
...
A különböző Azure SQL-termékek közötti tisztázásnak egyszerűsítenie és egyszerűsítenie kell az Azure-beli SQL Server-adatbázismotorral való munkát, legyen szó egyetlen felügyelt adatbázisról az Azure SQL Database-ban, egy teljes értékű felügyelt példányról, amely több adatbázist üzemeltet az Azure SQL Managed Instance-ban, vagy pedig az Azure-beli virtuális gépen üzemeltetett, jól ismert helyszíni SQL Server-terméket.
...
|---------|---------|---------|
|**Felügyelt Azure SQL-példány** | Azure SQL Database *példány*| Azure SQL Managed Instance egy saját termék a Azure SQL termékcsaládon belül, és nem csak egy üzembehelyhely helyezési lehetőség a Azure SQL Database. | 
|**Azure SQL Database**|Azure SQL Database adatbázis *létrehozása*| Hacsak nincs explicit módon másként megadva, a Azure SQL Database magában foglalja a rugalmas készletben üzembe helyezett adatbázisokat és adatbázisokat is. |
|**Azure SQL Database**|Azure SQL Database *készlet*| Hacsak nincs explicit módon másként megadva, a Azure SQL Database magában foglalja az egyedi adatbázisokat és a rugalmas készletben üzembe helyezett adatbázisokat is.  |
...
| **Azure SQL**| N/A | Ez az Azure SQL Server ban elérhető adatbázismotor-termékek Azure SQL Database, Azure SQL Managed Instance és SQL Server az Azure-beli virtuális gépeken. | 

## <a name="features-in-public-preview"></a>A nyilvános előzetes verzió funkciói
...
| Rugalmas lekérdezések | További információ: Elastic query overview (A rugalmas [lekérdezések áttekintése).](elastic-query-overview.md) |
| Rugalmas tranzakciók | [Elosztott tranzakciók a felhőalapú adatbázisok között.](elastic-transactions-overview.md) |
...
| [Példánykészletek](/azure/sql-database/sql-database-instance-pools) | A kisebb SQL-példányok felhőbe való áttelepítésének kényelmes és költséghatékony módja. |
...
- [A szolgáltatásokkal segített](https://azure.microsoft.com/updates/service-aided-subnet-configuration-for-managed-instance-in-azure-sql-database-available/) alhálózat-konfigurációval biztonságos és kényelmes módon kezelhetők az alhálózati konfigurációk, amelyekben ön szabályozhatja az adatforgalmat, SQL Managed Instance a felügyeleti forgalom zavartalan áramlását biztosítja.
- [transzparens adattitkosítás (TDE) és a Bring Your Own Key (BYOK)](https://azure.microsoft.com/updates/general-avilability-transparent-data-encryption-with-customer-managed-keys-for-azure-sql-database-managed-instance/) lehetővé teszi a saját kulcs használata (BYOK) forgatókönyvet az adatvédelemhez az adatokhoz, és lehetővé teszi a szervezetek számára, hogy elkülönítik a kulcsok és adatok felügyeleti feladatait.
...
A H1 2019-ben SQL Managed Instance alábbi funkciók vannak engedélyezve az üzembe helyezési modellben:
  - Azure-beli havi kreditet az <a href="/azure/azure-sql/managed-instance/resource-limits">előfizetőknek Visual Studio és</a> a megnövekedett regionális korlátok [támogatása.](../managed-instance/resource-limits.md#regional-resource-limitations)
  - A <a href="/sharepoint/administration/deploy-azure-sql-managed-instance-with-sharepoint-servers-2016-2019"> SharePoint 2016 és a SharePoint 2019 </a> és <a href="/business-applications-release-notes/october18/dynamics365-business-central/support-for-azure-sql-database-managed-instance"> a Dynamics 365 Business Central támogatása. </a>
...
|[Az elosztott tranzakciók a felügyelt példány kiszolgálói megbízhatósági csoportból való eltávolítása után hajthatóak végre](#distributed-transactions-can-be-executed-after-removing-managed-instance-from-server-trust-group)|2020. október|Áthidaló megoldással rendelkezik||
|[Az elosztott tranzakciók nem hajthatóak végre a felügyelt példány skálázása után](#distributed-transactions-cannot-be-executed-after-managed-instance-scaling-operation)|2020. október|Áthidaló megoldással rendelkezik||
|[BULK INSERT](/sql/t-sql/statements/bulk-insert-transact-sql) / A felügyelt példányban Azure SQL és utasítás [openROWSET](/sql/t-sql/functions/openrowset-transact-sql) utasítása nem használhatja az `BACKUP` / `RESTORE` Azure AD manage identityt az Azure Storage-ban való hitelesítéshez|2020. szeptember|Áthidaló megoldással rendelkezik||
...
|[A manuális biztonsági mentés ELLENŐRZŐUM nélküli visszaállítása meghiúsulhat](#restoring-manual-backup-without-checksum-might-fail)|2020. május|Feloldva|2020. június|
...
|[Az SQL Agent-feladatok az ügynökfolyamat újraindításával szakíthatóak meg](#sql-agent-jobs-can-be-interrupted-by-agent-process-restart)|2019. dec.|Feloldva|2020. márc.|
...
|[Resource Governor szolgáltatásszinten üzletileg kritikus szükséges lehet a feladatátvételt követően újrakonfigurálva](#resource-governor-on-business-critical-service-tier-might-need-to-be-reconfigured-after-failover)|2019. szeptember|Áthidaló megoldással rendelkezik||
...
|[@query paraméter nem támogatott a sp_send_db_mail](#-parameter-not-supported-in-sp_send_db_mail)|2019. április|Feloldva|2021. január|
|[A tranzakciós replikációt újra kell konfigurálni a geo-feladatátvétel után](#transactional-replication-must-be-reconfigured-after-geo-failover)|2019. márc.|Nincs áthidaló megoldás||
...
|[A tárterület túllépése kis adatbázisfájlokkal](#exceeding-storage-space-with-small-database-files)||Áthidaló megoldással rendelkezik||
...
|[Az ugyanazon példányon belüli két adatbázis tranzakciós hatóköre nem támogatott](#transaction-scope-on-two-databases-within-the-same-instance-isnt-supported)||Áthidaló megoldással rendelkezik|2020. márc.|
...
|Ha a forrásadatbázis memóriában lévő OLTP-objektumokat tartalmaz üzletileg kritikus adatbázis-visszaállítást általános célú szintről egy szintre, az időponthoz időben való visszaállítás nem fog sikerülni.||Feloldva|2019. október|
...
### <a name="changing-the-connection-type-does-not-affect-connections-through-the-failover-group-endpoint"></a>A kapcsolat típusának módosítása nincs hatással a feladatátvételi csoport végponton keresztüli kapcsolatokra
...
**Megkerülő** megoldás: Az automatikus feladatátvételi csoport elejtését és a csoport újraküldését a kapcsolat típusának módosítására kell halasztani.
...
A sp_send_dbmail paraméter használata esetén az eljárás átmenetien `@query` meghiúsulhat. Ha ez a probléma bekövetkezik, az eljárás minden második végrehajtása sp_send_dbmail és a következő `Msg 22050, Level 16, State 1` üzenettel `Failed to initialize sqlcmd library with error number -2147467259` meghiúsul: . A hiba megfelelő láthatóása érdekében az eljárást a paraméter alapértelmezett 0 értékével kell meghívni, ellenkező esetben a hiba propagálása `@exclude_query_output` nem történik meg.
...
A probléma megoldásához csomagolja be az e-mailek küldésére vonatkozó kódot egy újrapróbálkozási logikába, amely a kimeneti paraméterre `@mailitem_id` támaszkodik. Ha a végrehajtás sikertelen, akkor a paraméter értéke NULL lesz, ami azt sp_send_dbmail, hogy a rendszer még egyszer meg kell hívatni egy e-mail sikeres elküldését. Lássunk egy példát erre az újrapróbálkozási logikára.
```sql
...
### <a name="distributed-transactions-can-be-executed-after-removing-managed-instance-from-server-trust-group"></a>Az elosztott tranzakciók a felügyelt példány kiszolgálói megbízhatósági csoportból való eltávolítása után hajthatóak végre

[A kiszolgálói megbízhatósági csoportok](../managed-instance/server-trust-group-overview.md) segítségével megbízhatósági kapcsolatot lehet létesíteni a felügyelt példányok között, amely előfeltétel az elosztott [tranzakciók létrehozására.](./elastic-transactions-overview.md) Miután eltávolította a felügyelt példányt a kiszolgálói megbízhatósági csoportból, vagy törölte a csoportot, akkor is végrehajthat elosztott tranzakciókat. Íme egy áthidaló megoldás, amely segítségével győződjön meg [](../managed-instance/user-initiated-failover.md) arról, hogy az elosztott tranzakciók le vannak tiltva, és ez a felügyelt példányon a felhasználó által kezdeményezett manuális feladatátvétel.

### <a name="distributed-transactions-cannot-be-executed-after-managed-instance-scaling-operation"></a>Az elosztott tranzakciók nem hajthatóak végre a felügyelt példány skálázásának művelete után

A felügyelt példányok skálázásának műveletei, amelyek magukban foglalják a szolgáltatásszint vagy a virtuális magok számának megváltoztatását, visszaállítják a kiszolgálói megbízhatósági csoport beállításait a háttéren, és letiltják az elosztott [tranzakciók futtatását.](./elastic-transactions-overview.md) Áthidaló megoldásként törölje és hozza létre az új [kiszolgálói megbízhatósági csoportot](../managed-instance/server-trust-group-overview.md) a Azure Portal.
...
A tömeges beszúrási, BACKUP és RESTORE utasítások, valamint az OPENROWSET függvény nem használható felügyelt identitással az `DATABASE SCOPED CREDENTIAL` Azure Storage-ban való hitelesítéshez. Áthidaló megoldásként váltson a KÖZÖS HOZZÁFÉRÉSŰ JOGOSULTSÁG ALÁÍRÁSA hitelesítésre. Az alábbi példa nem fog működni a Azure SQL (az adatbázison és a felügyelt példányon is):
...
**Megkerülő** megoldás: A [tárolóban való hitelesítéshez használjon közös hozzáférésű jogosultsági aláírást.](/sql/t-sql/statements/bulk-insert-transact-sql#f-importing-data-from-a-file-in-azure-blob-storage)
...
Bizonyos körülmények között probléma lehet az Azure AD és a Azure Key Vault (AKV) szolgáltatások eléréséhez használt szolgáltatásnévvel. Ennek eredményeképpen ez a probléma hatással van az Azure AD-hitelesítésre és a transzparens adatbázis-titkosításra (TDE) a SQL Managed Instance. Ez időszakos csatlakozási problémaként vagy olyan utasítások nem futtatható, mint a CREATE LOGIN/USER FROM EXTERNAL PROVIDER vagy az EXECUTE AS LOGIN/USER. A TDE ügyfél által felügyelt kulccsal való beállítása egy új Azure SQL Managed Instance bizonyos körülmények között előfordulhat, hogy nem fog működni.

**Megkerülő** megoldás: Ha meg szeretné akadályozni, hogy ez a probléma a SQL Managed Instance-ban a frissítési parancsok végrehajtása előtt forduljon elő, vagy ha már tapasztalt ilyen problémát a frissítési parancsok után, a Azure Portal-hez, a SQL Managed Instance [Active Directory felügyeleti](./authentication-aad-configure.md?tabs=azure-powershell#azure-portal)paneljéhez. Ellenőrizze, hogy látja-e a következő hibaüzenetet: "A felügyelt példánynak szolgáltatásnévre van szüksége a Azure Active Directory. Kattintson ide a szolgáltatásnév létrehozásához". Ha ezt a hibaüzenetet észlelte, kattintson rá, és kövesse a megadott részletes utasításokat a hiba megoldásáig.

### <a name="restoring-manual-backup-without-checksum-might-fail"></a>A manuális biztonsági mentés ELLENŐRZŐUM nélküli visszaállítása sikertelen lehet

Bizonyos körülmények között előfordulhat, hogy a felügyelt példányon, ELLENŐRZŐUM nélkül készült adatbázisok manuális biztonsági mentése sikertelen lehet. Ilyen esetekben próbálja meg újra visszaállítani a biztonsági másolatot, amíg sikerrel nem jár.

**Megkerülő** megoldás: Manuális biztonsági mentés készítése felügyelt példányon található adatbázisokról a CHECKSUM engedélyezése esetén.
...
Bizonyos körülmények között egy meglévő feladat módosítása, letiltása vagy engedélyezése azt okozhatja, hogy az ügynök nem válaszol. A rendszer észleléskor automatikusan enyhíti a problémát, ami az ügynökfolyamat újraindítását eredményezi.
...
**Áthidaló** megoldás: Állítson be SQL Managed Instance közreműködői szerepkört a felhasználók számára az előfizetés szintjén.
...
Ha egy feladatátvételi csoport több Azure-előfizetésben vagy erőforráscsoportban lévő példányra is kitér, a manuális feladatátvétel nem indítható el a feladatátvételi csoportban lévő elsődleges példányról.

**Megkerülő** megoldás: Kezdeményezzen feladatátvételt a portálon keresztül a földrajzi másodlagos példányról.
...
Ha nem rendszergazdai bejelentkezéseket ad hozzá az [SQL Agent](/sql/ssms/agent/sql-server-agent-fixed-database-roles)rögzített adatbázis-szerepköreihez, akkor fennáll a probléma, hogy explicit EXECUTE engedélyeket kell adni a fő tárolt eljárásoknak ahhoz, hogy ezek a bejelentkezések működjön. Ha ez a probléma merül fel, a következő hibaüzenet jelenik meg: "A rendszer megtagadta az EXECUTE engedélyt az objektumon<object_name> (Microsoft SQL Server, Hiba: 229)" üzenet jelenik meg.
...
**(2020 márciusában megoldva)** Az SQL Agent minden egyes feladat munkához létrehoz egy új munkamenetet, ezzel fokozatosan növelve a memóriafelhasználást. Annak érdekében, hogy ne érje el a belső memóriakorlátot, amely blokkolná az ütemezett feladatok végrehajtását, az ügynökfolyamat akkor indul újra, ha a memóriafelhasználás eléri a küszöbértéket. Ez az újraindítás pillanatában futó feladatok végrehajtásának megszakítását eredményezheti.
...
**Megkerülő** megoldás: Figyelje a memóriában található [OLTP-tárhelyhasználatot](../in-memory-oltp-monitor-space.md) SQL Server Management Studio, hogy a számítási feladat ne használja a rendelkezésre álló memóriánál többet. [](/sql/relational-databases/in-memory-oltp/monitor-and-troubleshoot-memory-usage#bkmk_Monitoring) Növelje a virtuális magok számától függő memóriakorlátokat, vagy optimalizálja a számítási feladatot úgy, hogy kevesebb memóriát használjon.
...
SQL Server és SQL Managed Instance nem teszik lehetővé, hogy a felhasználó nem üres fájlt [hagyjon el.](/sql/relational-databases/databases/delete-data-or-log-files-from-a-database#Prerequisites) Ha egy utasítással próbál eltávolítani egy nem régi adatfájlt, a rendszer nem ad `ALTER DATABASE REMOVE FILE` `Msg 5042 – The file '<file_name>' cannot be removed because it is not empty` vissza azonnal hibát. SQL Managed Instance megpróbálja eldobni a fájlt, és a művelet 30 perc után sikertelen lesz a `Internal server error` következővel: .
...
A folyamatban lévő utasítás, a Data Migration Service migrálási folyamata és a beépített, időponthoz időben végzett visszaállítás letiltja a szolgáltatási szint frissítését vagy a meglévő példány átméretezését és új példányok létrehozását a visszaállítási folyamat `RESTORE` befejezéséig. 

A visszaállítási folyamat blokkolja ezeket a műveleteket a felügyelt példányok és példánykészletek számára ugyanazon az alhálózaton, ahol a visszaállítási folyamat fut. A példánykészletben található példányok nem érintettek. A szolgáltatásszint-műveletek létrehozása vagy módosítása nem fog meghiúsulni vagy időkorrekciót létrehozni. A visszaállítási folyamat befejezése vagy megszakítása után folytatódnak.

**Megkerülő** megoldás: Várja meg, amíg a visszaállítási folyamat befejeződik, vagy ha a létrehozási vagy frissítési szolgáltatási szint művelete magasabb prioritással rendelkezik, állítsa le a visszaállítási folyamatot.

### <a name="resource-governor-on-business-critical-service-tier-might-need-to-be-reconfigured-after-failover"></a>Resource Governor a üzletileg kritikus szolgáltatási szinten, előfordulhat, hogy a feladatátvételt követően újra kell konfigurálni

A [Resource Governor](/sql/relational-databases/resource-governor/resource-governor) szolgáltatás, amely lehetővé teszi a felhasználói számítási feladathoz rendelt erőforrások korlátozását, helytelenül besorolhat egyes felhasználói számítási feladatokat a feladatátvétel vagy a szolgáltatási szint felhasználó által kezdeményezett módosítása után (például a virtuális magok maximális tárterületének módosítása vagy a példányok maximális tárterületméretének módosítása).

**Megkerülő** megoldás: Rendszeres időközönként vagy egy SQL Agent-feladat részeként futtassa a következőt: , amely végrehajtja az SQL-feladatot, amikor a példány elindul, ha `ALTER RESOURCE GOVERNOR RECONFIGURE` Resource Governor. [](/sql/relational-databases/resource-governor/resource-governor)
...
Az adatbázisközi Service Broker párbeszédpanelek a szolgáltatási szint módosítása után nem kézbesítik az üzeneteket más adatbázisokban a szolgáltatásoknak. Az üzenetek nem *elvesznek,* és a küldő üzenetsorában találhatók. A virtuális magok vagy a példány tárterületméretének SQL Managed Instance hatására a `service_broke_guid` [sys.databases](/sql/relational-databases/system-catalog-views/sys-databases-transact-sql) nézetben az összes adatbázis értéke módosul. A más adatbázisokban service brokerre hivatkozó BEGIN DIALOG utasítással létrehozott üzenetek nem kézbesítenek üzeneteket `DIALOG` a célszolgáltatásnak. [](/sql/t-sql/statements/begin-dialog-conversation-transact-sql)

**Megkerülő** megoldás: Állítson le minden olyan tevékenységet, amely adatbázisközi párbeszédablakokat Service Broker a szolgáltatási szint frissítése előtt, majd ezt követően újra kell indítania azokat. Ha vannak olyan üzenetek, amelyek nincsenek kézbesítve a szolgáltatási szint módosítása után, olvassa be az üzeneteket a forrás üzenetsorból, és küldje el újra a cél üzenetsorba.
...
### <a name="query-parameter-not-supported-in-sp_send_db_mail"></a>@query A paraméter nem támogatott a sp_send_db_mail
...
Amikor egy adatbázis visszaállítása a SQL Managed Instance, a visszaállítási szolgáltatás először létrehoz egy üres adatbázist a kívánt névvel a név példányon való lefoglalásához. Egy idő után a rendszer eldobja ezt az adatbázist, és a tényleges adatbázis visszaállítása meg fog elindulni. 

A Visszaállítás állapotban van *az* adatbázis neve helyett ideiglenesen egy véletlenszerű GUID-érték lesz. A visszaállítási folyamat befejezése után az ideiglenes név az utasításban megadott kívánt `RESTORE` névre módosul. 
...
Az adatbázis mindig 12 adatfájlra van felosztva, és a `tempdb` fájlstruktúra nem módosítható. A fájlonkénti maximális méret nem módosítható, és nem lehet új fájlokat hozzáadni a `tempdb` fájlhoz. `Tempdb` A mindig üres adatbázisként jön létre, amikor a példány elindul vagy átveszi a adatokat, és a-ban végrehajtott módosítások nem `tempdb` maradnak meg.

### <a name="exceeding-storage-space-with-small-database-files"></a>A tárterület túllépése kis méretű adatbázisfájlokkal

`CREATE DATABASE`, `ALTER DATABASE ADD FILE` és utasítások `RESTORE DATABASE` sikertelenek lehetnek, mert a példány elérheti az Azure Storage korlátját.

A általános célú példányok SQL Managed Instance 35 TB tárterületet foglalnak le az Azure Premium Disk-tárhely számára. Minden adatbázisfájl külön fizikai lemezre kerül. A lemezek mérete 128 GB, 256 GB, 512 GB, 1 TB vagy 4 TB lehet. A lemez fel nem használt tárhelye nem számít fel díjat, de az Azure Premium Disk-méretek teljes összege nem haladhatja meg a 35 TB-t. Bizonyos esetekben a belső töredezettség miatt előfordulhat, hogy egy felügyelt példánynak összesen nem lesz szüksége 8 TB-ra, és meghaladja a tárterület méretére vonatkozó 35 TB-os Azure-korlátot.

Egy virtuális általános célú például SQL Managed Instance egy 4 TB méretű, 1,2 TB méretű fájlt is elhelyezhet. Emellett 248, 1 GB méretű fájlt is tartalmazhat, amelyek külön 128 GB-os lemezeken vannak elhelyezve. Ebben a példában:

- A lefoglalt lemez teljes tárterületmérete 1 x 4 TB + 248 x 128 GB = 35 TB.
- A példányon található adatbázisok teljes fenntartott tárterülete 1 x 1,2 TB + 248 x 1 GB = 1,4 TB.

Ez a példa azt mutatja be, hogy bizonyos körülmények között a fájlok egy adott elosztása miatt az SQL Managed Instance egy példánya elérheti a csatolt Prémium szintű Azure-lemez számára lefoglalt 35 TB-os korlátot, ha várhatóan nem.

Ebben a példában a meglévő adatbázisok továbbra is működnek, és probléma nélkül növekedhetnek, ha nem adnak hozzá új fájlokat. Nem lehet új adatbázisokat létrehozni vagy visszaállítani, mert nincs elegendő hely az új lemezmeghajtók számára, még akkor sem, ha az összes adatbázis teljes mérete nem éri el a példány méretkorlátját. Az ebben az esetben visszaadott hiba nem egyértelmű.

A [rendszernézetek segítségével](https://medium.com/azure-sqldb-managed-instance/how-many-files-you-can-create-in-general-purpose-azure-sql-managed-instance-e1c7c32886c1) azonosíthatja a fennmaradó fájlok számát. Ha eléri ezt a [](/sql/t-sql/database-console-commands/dbcc-shrinkfile-transact-sql#d-emptying-a-file) korlátot, próbálja meg kiüríteni és törölni néhány kisebb fájlt a DBCC SHRINKFILE-utasítással, vagy váltson át a [üzletileg kritikus-szintre,](../managed-instance/resource-limits.md#service-tier-characteristics)amely nem tartalmazza ezt a korlátot.

### <a name="guid-values-shown-instead-of-database-names"></a>Adatbázisnevek helyett megjelenő GUID-értékek

Számos rendszernézet, teljesítményszámláló, hibaüzenet, XEvent és hibanapló bejegyzés a tényleges adatbázisnevek helyett GUID adatbázis-azonosítókat jelenít meg. Ne támaszkodjon ezekre a GUID-azonosítókra, mert a jövőben tényleges adatbázisnevekre cseréljük őket.

**Megkerülő** megoldás: A sys.databases nézet használatával oldja fel a tényleges adatbázisnevet a fizikai adatbázis nevéből, amely GUID-adatbázisazonosítók formájában van megadva:
...
Az egyes SQL Managed Instance elérhető hibanaplók nem maradnak meg, és a méretük nem szerepel a maximális tárhelykorlátban. Feladatátvétel esetén előfordulhat, hogy a hibanaplók automatikusan törlődnek. A hibanapló előzményei között lehetnek hiányosságok, SQL Managed Instance több virtuális gépen többször is áthelyezte a rendszer.

### <a name="transaction-scope-on-two-databases-within-the-same-instance-isnt-supported"></a>Az egy példányon belüli két adatbázis tranzakciós hatóköre nem támogatott

**(2020 márciusában megoldva)** A .NET osztálya nem működik, ha két lekérdezést küld két adatbázisnak ugyanazon példányon belül, ugyanabban a `TransactionScope` tranzakciós hatókörben:
...
**Áthidaló megoldás (2020.** március óta nincs rá szükség): Az [SqlConnection.ChangeDatabase(String)](/dotnet/api/system.data.sqlclient.sqlconnection.changedatabase) használatával egy másik adatbázist használhat kapcsolati környezetben két kapcsolat használata helyett.
...
Az aktuális példányra hivatkozó SQL Managed Instance és csatolt kiszolgálók CLR-moduljai vagy elosztott lekérdezései néha nem oldják fel a helyi példány IP-címét. Ez a hiba átmeneti probléma.
...
A dokumentációban való közreműködéshez Azure SQL lásd a [Docs-szerkesztői útmutató.](/contribute/)
