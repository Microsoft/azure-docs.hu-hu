...
> [!CAUTION]
> Az útválasztási viselkedés a DNN használata esetén eltérő, ha VNN használ. Ne használja a 1433-es portot. További tudnivalókat a jelen cikk későbbi, a [port megfontolása](#port-considerations) című szakaszában talál.
...
- Azonosítsa a DNN-figyelőhöz használni kívánt egyedi portot. A DNN-figyelőhöz használt portnak egyedinek kell lennie a rendelkezésre állási csoport vagy a feladatátvevő fürt példányának összes replikáján.  Egyetlen másik csatlakozás sem osztható meg ugyanazzal a porttal.
...
- Az DNN-figyelőket egyedi porttal **kell konfigurálni** .  A portot nem lehet megosztani bármely replikán lévő más csatlakozással.
...
## <a name="port-considerations"></a>A portok szempontjai

A DNN-figyelők az összes IP-címet figyelik, de egy adott egyedi porton. A figyelő nevének DNS-bejegyzésének fel kell oldania a rendelkezésre állási csoport összes replikájának címét. Ez automatikusan történik a [create script (parancsfájl létrehozása](#create-script) ) szakaszban megadott PowerShell-parancsfájllal. Mivel a DNN-figyelők minden IP-címen fogadnak kapcsolatokat, kritikus fontosságú, hogy a figyelő portja egyedi legyen, és a rendelkezésre állási csoportban lévő többi replika ne használja. Mivel SQL Server mindig a 1433-as porton figyeli közvetlenül vagy a SQL Browser szolgáltatáson keresztül, a 1433-es port nem használható egyetlen DNN-figyelőhöz sem.
