...
1. A bemeneti üzenet formázása XML-borítékkal. Példaként tekintse meg a következő példa XML-adattartalmat:
...
Ha a **(z) "sapgw00" szolgáltatáshoz** hasonló üzenet jelenik meg a 500-es **hibás átjárónál** , cserélje le az ÁTJÁRÓ szolgáltatás nevét az API-kapcsolatra, és aktiválja a konfigurációt a portszámával. A következő példában `sapgw00` egy valós portszámot kell cserélni, például: `3300` . 
...
##### <a name="the-requestcontext-on-the-ireplychannel-was-closed-without-a-reply-being-sent"></a>A IReplyChannel RequestContext a küldési válasz nélkül bezárult

Ez a hibaüzenet olyan váratlan hibákat jelez, amikor a csatorna catch-all kezelője egy hiba miatt leállítja a csatornát, és újraépíti a csatornát más üzenetek feldolgozására.

Ha szeretné megismerni, hogy a logikai alkalmazás fogadta a IDoc, [adjon hozzá egy](../connectors/connectors-native-reqres.md#add-a-response-action) , az állapotkódot visszaadó Response műveletet `200 OK` . Hagyja üresen a törzset, és ne módosítsa vagy ne adja hozzá a fejléceket. A IDoc a tRFC-en keresztül történik, ami nem teszi lehetővé a válasz adattartalommal való elszállítását.

A IDoc elutasításához válaszoljon a-tól eltérő HTTP-állapotkódot `200 OK` . Az SAP-adapter ezután egy kivételt ad vissza az SAP-nek az Ön nevében. Csak akkor utasítsa el a IDoc, ha a továbbítási hibákat az SAP felé szeretné irányítani, például egy olyan nem irányított IDoc, amelyet az alkalmazás nem tud feldolgozni. Nem szabad visszautasítani egy IDoc az alkalmazás szintű hibákhoz, például a IDoc tárolt adatproblémákkal kapcsolatban. Ha késlelteti az alkalmazás-szintű ellenőrzés elfogadását, előfordulhat, hogy negatív teljesítményt tapasztal, mert blokkolja a kapcsolatokat más IDocs szállításával.

Ha ezt a hibaüzenetet kapja, és rendszerszintű hibákat tapasztal Logic Apps meghívásával, ellenőrizze, hogy konfigurálta-e a helyszíni adatátjáró szolgáltatás hálózati beállításait az adott környezethez. Ha például a hálózati környezet egy proxy használatát igényli az Azure-végpontok meghívásához, akkor konfigurálnia kell a helyszíni adatátjáró szolgáltatást a proxy használatára. További információ: [proxy konfigurálása](/dotnet/framework/network-programming/proxy-configuration).

Ha ezt a hibaüzenetet kapja, és a Logic Apps meghívásakor időnként előforduló hibák fordulnak elő, előfordulhat, hogy az újrapróbálkozások számának és/vagy újrapróbálkozás intervallumának növelésére van szükség. 

1. A helyszíni adatátjáró szolgáltatás konfigurációs fájljában keresse meg az SAP-beállításokat `Microsoft.PowerBI.EnterpriseGateway.exe.config` . Az újrapróbálkozások száma beállítás úgy néz ki, mint `WebhookRetryMaximumCount="2"` . Az újrapróbálkozási időköz beállítása a következőképpen néz ki: `WebhookRetryDefaultDelay="00:00:00.10"` és a TimeSpan formátuma `HH:mm:ss.ff` . 
    
1. Mentse a módosításokat, és indítsa újra a helyszíni adatátjárót.

##### <a name="the-segment-or-group-definition-e2edk36001-was-not-found-in-the-idoc-meta"></a>A szegmens vagy a csoport definíciójának E2EDK36001 nem található a IDoc meta-ben.

Ez a hibaüzenet azt jelenti, hogy a várt hibák más hibákkal történnek. Például egy IDoc XML-adattartalom létrehozása nem sikerült, mert a szegmenseit nem az SAP bocsátja ki. Ennek eredményeképpen az átalakításhoz szükséges szegmens típusú metaadatok hiányoznak.
