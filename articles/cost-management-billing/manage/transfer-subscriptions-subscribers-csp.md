...
Ha rendelkezik meglévő foglalásokkal, azok az előfizetés átvitele után 90 nappal leállnak. A 90 [napos](../reservations/exchange-and-refund-azure-reservations.md) türelmi időszak utáni díjak elkerülése érdekében mindenképpen törölje a foglalásokat, és az előfizetés átadása előtt térítse vissza őket.
...
1. Viszonteladói [kapcsolat létrehozása az](/partner-center/request-a-relationship-with-a-customer) ügyféllel. Tekintse át a CSP regionális hitelesítés [áttekintését,](/partner-center/regional-authorization-overview) és győződjön meg arról, hogy az ügyfél és a partner bérlője is ugyanabban az engedélyezett régióban található.
...
Bármely más előfizetés egy felhőszolgáltatói partnertől bármely egyéb Azure-ajánlatba történő áthelyezésekor az előfizetőnek erőforrásokat kell áthelyeznie a felhőszolgáltatói forrás-előfizetések és a célelőfizetések között. Ezt egy partner és egy ügyfél végez, a Microsoft képviselőjének nem.

1. Az ügyfél cél Azure-előfizetéseket hoz létre.
...
1. Az átvitelhez használt ügyfélfelhasználói fióknak [Azure RBAC-tulajdonosi](add-change-subscription-administrator.md) hozzáféréssel kell lennie mindkét előfizetéshez.
1. A kezdés előtt [ellenőrizze](/rest/api/resources/resources/validatemoveresources), hogy az összes Azure-erőforrás áthelyezhető-e a forrás-előfizetésből a célelőfizetésbe.
    > [!IMPORTANT]
    >  - Bizonyos Azure-erőforrások nem helyezhetők át az előfizetések között. Az áthelyezhető Azure-erőforrások teljes listájának megtekintéséhez lásd: [Áthelyezési művelet támogatása erőforrásokhoz](../../azure-resource-manager/management/move-support-resources.md).
