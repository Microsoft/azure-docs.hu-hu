...
description: Rövid útmutató – Leírja, hogyan állíthatja be gyorsan a Azure NetApp Files és hozhat létre kötetet.
author: b-juche
ms.author: b-juche
...
Ez a cikk bemutatja, hogyan állíthatja be gyorsan a Azure NetApp Files és hozhat létre NFS-kötetet. 

Ebben a rövid útmutatóban a következő elemeket fogja beállítani:

- Regisztráció a Azure NetApp Files NetApp erőforrás-szolgáltatóhoz
- NetApp-fiók
- Kapacitáskészlet
- NfS-kötet Azure NetApp Files
...
> [!IMPORTANT]
> Hozzáférést kell biztosítani a Azure NetApp Files szolgáltatáshoz. A szolgáltatáshoz való hozzáférés igénylését lásd a Azure NetApp Files [várakozási lista beküldési oldalán.](https://aka.ms/azurenetappfiles)  A folytatás előtt meg kell várnia a csapattól Azure NetApp Files visszaigazoló e-mailt.

---

## <a name="register-for-azure-netapp-files-and-netapp-resource-provider"></a>Regisztráció a Azure NetApp Files NetApp erőforrás-szolgáltatóra

> [!NOTE]
> A regisztrációs folyamat némi időt vehet igénybe.
>
...
A cikkhez az az Azure PowerShell Az 2.6.0-s vagy újabb verziójára lesz szükség. Az aktuális verzió azonosításához futtassa a következőt: `Get-Module -ListAvailable Az`. Ha telepíteni vagy frissíteni szeretne, olvassa el [az Azure PowerShell-modul telepítését](/powershell/azure/install-Az-ps) ismertető cikket. Ha szeretné, használhatja a Cloud Shell egy PowerShell-munkamenetben.

1. Egy PowerShell-parancssorban (vagy PowerShell Cloud Shell-munkamenetben) adja meg azt az előfizetést, amely számára jóvá lett hagyva a Azure NetApp Files:
...
A Azure Portal, a PowerShell vagy az Azure CLI használatával regisztráljon Azure NetApp Files NetApp erőforrás-szolgáltatóra.

További [információ: Azure NetApp Files](azure-netapp-files-register.md) regisztrálása a regisztrációra.
...
1. Határozzon meg néhány változót, hogy a többi példában is hivatkozni tudjuk rájuk:
...
2. Hozzon Azure NetApp Files fiókot a [New-AzNetAppFilesAccount paranccsal:](/powershell/module/az.netappfiles/New-AzNetAppFilesAccount)
...
1. Határozzon meg néhány változót, hogy a többi példában is hivatkozni tudjuk rájuk:
    ...
    > [!NOTE]
    > A támogatott [régiók listájáért tekintse](https://azure.microsoft.com/global-infrastructure/services/?products=netapp&regions=all) meg az elérhető termékek régiónkénti listáját.
...
2. Hozzon létre egy új erőforráscsoportot [az az group create paranccsal:](/cli/azure/group#az_group_create)
...
3. Hozzon Azure NetApp Files az [az netappfiles account create paranccsal:](/cli/azure/netappfiles/account#az_netappfiles_account_create)
...
1. A Azure NetApp Files panelen válassza ki NetApp-fiókját **(myaccount1).**
...
4. Adja meg a kapacitáskészlet adatait:
    ...
    * Szolgáltatási **szintként** válassza a Prémium lehetőséget.
...
2. Új kapacitáskészlet létrehozása az [az netappfiles pool create használatával](/cli/azure/netappfiles/pool#az_netappfiles_pool_create)
...
3. A Kötet létrehozása ablakban adja meg a kötet adatait:
   ...
   3. A kvótához használja az alapértelmezett értéket.
   4. A virtuális hálózat alatt kattintson az **Új létrehozása** elemre egy új Azure-beli virtuális hálózat (Vnet) létrehozásához.  Ezután adja meg a következő adatokat:
       ...
       * Az **alhálózat neveként adja meg a myANFsubnet** nevet.
       * Adja meg az alhálózat címtartományát, például: 10.7.0.0/24. A dedikált alhálózatot nem oszthatja meg más erőforrásokkal.
       * Az **alhálózat delegáláshoz válassza a Microsoft.NetApp/volumes** lehetőséget.
       * A **virtuális hálózat létrehozásához** kattintson az OK gombra.
...
1. Hozzon létre virtuális hálózatot alhálózat nélkül az [az network vnet create paranccsal.](/cli/azure/network/vnet#az_network_vnet_create)
...
2. Hozzon létre egy delegált alhálózatot [az az network vnet subnet create paranccsal.](/cli/azure/network/vnet/subnet#az_network_vnet_subnet_create)
...
3. Hozza létre a kötetet [az az netappfiles volume create paranccsal.](/cli/azure/netappfiles/volume#az_netappfiles_volume_create)
...
3. Az erőforráscsoport oldalán kattintson az **Erőforráscsoport törlése elemre.**
    ...
    ![Képernyőkép az Erőforráscsoport törlése gomb kiemelésről.](../media/azure-netapp-files/azure-netapp-files-azure-delete-resource-group.png)

    Ekkor megnyílik egy ablak, mely figyelmezteti az erőforráscsoporttal törölt erőforrásokról.

4. Adja meg az erőforráscsoport (myRG1) nevét annak megerősítéséhez, hogy véglegesen törölni szeretné az erőforráscsoportot és a benne benne álló összes erőforrást, majd kattintson a **Törlés gombra.**
...
1. Törölje az erőforráscsoportot [az az group delete paranccsal.](/cli/azure/group#az_group_delete)
