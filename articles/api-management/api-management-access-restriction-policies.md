...
| hívások          | A által megadott időintervallumban engedélyezett hívások maximális száma összesen `renewal-period` | Igen      | N/A     |
| megújítás – időszak | A lecsúszó ablak hossza (másodpercben), amely alatt az engedélyezett kérelmek száma nem haladhatja meg a ben megadott értéket `calls` .                                              | Igen      | N/A     |
| újrapróbálkozást követő fejléc neve    | Annak a válasz fejlécnek a neve, amelynek értéke az ajánlott újrapróbálkozási időköz másodpercben a megadott hívási arány túllépése után. |  Nem | N/A  |
| újrapróbálkozást követő változó neve    | Egy olyan házirend-kifejezési változó neve, amely a megadott hívási arány túllépése után másodpercek alatt tárolja a javasolt újrapróbálkozási időközt. |  Nem | N/A  |
| hátralévő – hívások – fejléc – név    | A válasz fejlécének neve, amelynek értéke az egyes házirendek végrehajtása után a által megadott időintervallumban engedélyezett hátralévő hívások száma `renewal-period` . |  Nem | N/A  |
| fennmaradó hívások – változó – név    | Egy olyan házirend-kifejezési változó neve, amely az egyes házirendek végrehajtása után a alkalmazásban megadott időintervallumban engedélyezett hátralévő hívások számát tárolja `renewal-period` . |  Nem | N/A  |
| összes hívás – fejléc-név    | A válasz fejlécének neve, amelynek értéke a ben megadott érték `calls` . |  Nem | N/A  |
...
| növekmény – feltétel | Az a logikai kifejezés, amely megadja, hogy a kérést a ráta () felé kell-e számítani `true` .        | Nem       | N/A     |
| megújítás – időszak      | A lecsúszó ablak hossza (másodpercben), amely alatt az engedélyezett kérelmek száma nem haladhatja meg a ben megadott értéket `calls` .                                           | Igen      | N/A     |
| újrapróbálkozást követő fejléc neve    | Annak a válasz fejlécnek a neve, amelynek értéke az ajánlott újrapróbálkozási időköz másodpercben a megadott hívási arány túllépése után. |  Nem | N/A  |
| újrapróbálkozást követő változó neve    | Egy olyan házirend-kifejezési változó neve, amely a megadott hívási arány túllépése után másodpercek alatt tárolja a javasolt újrapróbálkozási időközt. |  Nem | N/A  |
| hátralévő – hívások – fejléc – név    | A válasz fejlécének neve, amelynek értéke az egyes házirendek végrehajtása után a által megadott időintervallumban engedélyezett hátralévő hívások száma `renewal-period` . |  Nem | N/A  |
...
| megújítás – időszak | Az az időtartam másodpercben, amely után a kvóta alaphelyzetbe áll. Ha `0` a pontra van állítva, a végtelen értékre van állítva. | Igen                                                              | N/A     |
