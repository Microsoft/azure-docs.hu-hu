...
| `wids` | [RoleTemplateID](../roles/permissions-reference.md#all-roles) GUID-azonosítók tömbje | Az ehhez a felhasználóhoz rendelt bérlői szerepköröket jelöli az [Azure ad beépített szerepköreiben](../roles/permissions-reference.md#all-roles)lévő szerepkörök szakasza alapján.  Ez a jogcím az `groupMembershipClaims` [alkalmazás jegyzékfájljának](reference-app-manifest.md)tulajdonságán alapuló, alkalmazáson belüli alapon van konfigurálva.  Az "all" vagy a "DirectoryRole" beállítás megadása kötelező.  Előfordulhat, hogy a jogkivonat-hossz miatti implicit folyamat során beszerzett jogkivonatok nem jelennek meg. |
...
| `oid` | Karakterlánc, GUID | A kérelem "résztvevője" számára megváltoztathatatlan azonosító – az a felhasználó vagy szolgáltatásnév, amelynek identitását ellenőrizték.  Az azonosító tokenek és az alkalmazás-és felhasználói tokenek esetében ez a felhasználó objektumazonosító.  Csak alkalmazási jogkivonatokban ez a hívó szolgáltatásnév objektumának azonosítója. Emellett az engedélyezési ellenőrzések biztonságos elvégzésére, valamint az adatbázis tábláiban lévő kulcsra is felhasználható. Ez az azonosító egyedileg azonosítja a rendszerbiztonsági tag-t az alkalmazások között – két különböző alkalmazás, amely ugyanazon a felhasználón jelentkezik be, ugyanazokat az értékeket kapja meg a `oid` jogcímben. Így `oid` a Microsoft online szolgáltatások (például a Microsoft Graph) lekérdezéseit is felhasználhatja. A Microsoft Graph `id` egy adott [felhasználói fiókhoz](/graph/api/resources/user)tartozó tulajdonságként adja vissza ezt az azonosítót. Mivel az `oid` lehetővé teszi több alkalmazás összekapcsolását a rendszerbiztonsági tag számára, a `profile` hatókörre azért van szükség, hogy a felhasználók megkapják ezt a jogcímet. Vegye figyelembe, hogy ha egyetlen felhasználó több bérlőn is létezik, akkor a felhasználó minden bérlőn egy másik objektumazonosítót fog tartalmazni – ezeket a rendszer akkor is eltérő fióknak tekinti, ha a felhasználó ugyanazzal a hitelesítő adatokkal jelentkezik be az egyes fiókokba. |
