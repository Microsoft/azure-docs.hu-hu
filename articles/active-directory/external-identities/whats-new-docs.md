---
title: A Azure Active Directory külső identitások újdonságai
description: Új és frissített dokumentáció a Azure Active Directory külső identitások számára.
ms.date: 03/08/2021
ms.service: active-directory
ms.subservice: B2B
ms.topic: reference
ms.workload: identity
ms.author: mimart
author: msmimart
manager: CelesteDG
ms.openlocfilehash: dda3f7c8f4a9a21e16ebab7d243a004c01a40eed
ms.sourcegitcommit: f28ebb95ae9aaaff3f87d8388a09b41e0b3445b5
ms.translationtype: MT
ms.contentlocale: hu-HU
ms.lasthandoff: 03/29/2021
ms.locfileid: "102453665"
---
# <a name="azure-active-directory-external-identities-whats-new"></a>Azure Active Directory külső identitások: Újdonságok

Üdvözli a Azure Active Directory külső identitások dokumentációjának újdonságai. Ez a cikk felsorolja a hozzáadott új dokumentumokat, valamint azokat, amelyek az elmúlt három hónapban jelentős frissítésekkel rendelkeztek. A külső identitások szolgáltatás újdonságait itt tekintheti meg: a [Azure Active Directory újdonságai](../fundamentals/whats-new.md).

## <a name="february-2021"></a>2021. február

### <a name="new-articles"></a>Új cikkek

- [Vendég felhasználó beváltási állapotának visszaállítása](reset-redemption-status.md)

### <a name="updated-articles"></a>Frissített cikkek

- [Azure Active Directory B2B – ajánlott eljárások](b2b-fundamentals.md)
- [Külső B2B-együttműködés engedélyezése és a vendégmeghívási jogosultság kezelése](delegate-invitations.md)
- [Azure Active Directory B2B-együttműködés – gyakori kérdések](faq.md)
- [Egyszeri jelszavas hitelesítés e-mailben](one-time-passcode.md)
- [Azure Active Directory B2B-együttműködésre irányuló meghívás beváltása](redemption-experience.md)
- [Azure Active Directory B2B-együttműködés hibaelhárítása](troubleshoot.md)
- [Egy Azure Active Directory B2B csoportmunka-felhasználó tulajdonságai](user-properties.md)
- [Mit jelent a vendégfelhasználók hozzáférése az Azure Active Directory B2B-ben?](what-is-b2b.md)
- [Azure Active Directory külső identitások: Újdonságok](whats-new-docs.md)
- [Adott szervezetek B2B-felhasználók felé irányuló meghívásainak engedélyezése vagy letiltása](allow-deny-list.md)
- [Azure Active Directory B2B együttműködési API és testreszabás](customize-invitation-api.md)
- [Belső felhasználók meghívása B2B együttműködésre](invite-internal-users.md)
- [Microsoft 365 külső megosztási és Azure Active Directory (Azure AD) B2B együttműködés](o365-external-user.md)
- [Közvetlen összevonás AD FS és külső szolgáltatókkal a vendég felhasználói számára (előzetes verzió)](direct-federation.md)

## <a name="january-2021"></a>2021. január

### <a name="updated-articles"></a>Frissített cikkek
- [Adott szervezetek B2B-felhasználók felé irányuló meghívásainak engedélyezése vagy letiltása](allow-deny-list.md)
- [Hogyan hívhatják meg a felhasználók a vendégeket egy alkalmazásba](add-users-information-worker.md)


## <a name="december-2020"></a>2020. december

### <a name="updated-articles"></a>Frissített cikkek
- [Azure Active Directory B2B-együttműködés – gyakori kérdések](faq.md)
- [Google hozzáadása identitás-szolgáltatóként a B2B vendég felhasználói számára](google-federation.md)
- [Külső identitások identitás-szolgáltatói](identity-providers.md)
- [Azure Active Directory B2B-együttműködésre irányuló meghívás beváltása](redemption-experience.md)
- [API-összekötő hozzáadása felhasználói folyamathoz](self-service-sign-up-add-api-connector.md)
- [Egyéni jóváhagyási munkafolyamat hozzáadása az önkiszolgáló regisztrációhoz](self-service-sign-up-add-approvals.md)
- [Azure Active Directory B2B-együttműködés hibaelhárítása](troubleshoot.md)
- [Mit jelent a vendégfelhasználók hozzáférése az Azure Active Directory B2B-ben?](what-is-b2b.md)
- [Azure Active Directory B2B – ajánlott eljárások](b2b-fundamentals.md)
- [Külső B2B-együttműködés engedélyezése és a vendégmeghívási jogosultság kezelése](delegate-invitations.md)
- [Egyszeri jelszavas hitelesítés e-mailben](one-time-passcode.md)

## <a name="november-2020"></a>2020. november

### <a name="updated-articles"></a>Frissített cikkek
- [Microsoft 365 külső megosztási és Azure Active Directory (Azure AD) B2B együttműködés](o365-external-user.md)
- [Hozzáférés biztosítása helyben kezelt partnerfiókoknak felhőbeli erőforrásokhoz az Azure AD vállalatközi együttműködéssel](hybrid-on-premises-to-cloud.md)
- [Egy Azure Active Directory B2B csoportmunka-felhasználó tulajdonságai](user-properties.md)


## <a name="october-2020"></a>2020. október

### <a name="updated-articles"></a>Frissített cikkek
- [Google hozzáadása identitás-szolgáltatóként a B2B vendég felhasználói számára](google-federation.md)
- [Hogyan hívhatják meg a felhasználók a vendégeket egy alkalmazásba](add-users-information-worker.md)
- [Adott szervezetek B2B-felhasználók felé irányuló meghívásainak engedélyezése vagy letiltása](allow-deny-list.md)
- [Azure Active Directory B2B-együttműködés – gyakori kérdések](faq.md)
- [Az External Identities dokumentációja](index.yml)
- [Azure Active Directory B2B-együttműködésre irányuló meghívás beváltása](redemption-experience.md)
- [Egyéni jóváhagyási munkafolyamat hozzáadása az önkiszolgáló regisztrációhoz](self-service-sign-up-add-approvals.md)
- [Mik a külső identitások a Azure Active Directoryban?](compare-with-b2c.md)
- [API-összekötő hozzáadása felhasználói folyamathoz](self-service-sign-up-add-api-connector.md)


## <a name="september-2020"></a>2020. szeptember

### <a name="updated-articles"></a>Frissített cikkek
- [A B2B együttműködés meghívójának e-mail-Azure Active Directory elemei](invitation-email-elements.md)
- [Azure Active Directory B2B-együttműködés hibaelhárítása](troubleshoot.md)
- [Számlázási modell az Azure AD külső identitásokhoz](external-identities-pricing.md)
- [Google hozzáadása identitás-szolgáltatóként a B2B vendég felhasználói számára](google-federation.md)


## <a name="august-2020"></a>2020. augusztus

### <a name="new-articles"></a>Új cikkek
- [Számlázási modell az Azure AD külső identitásokhoz](external-identities-pricing.md)


### <a name="updated-articles"></a>Frissített cikkek
- [Külső B2B-együttműködés engedélyezése és a vendégmeghívási jogosultság kezelése](delegate-invitations.md)
- [API-összekötő hozzáadása felhasználói folyamathoz](self-service-sign-up-add-api-connector.md)
- [Egyéni jóváhagyási munkafolyamat hozzáadása az önkiszolgáló regisztrációhoz](self-service-sign-up-add-approvals.md)


