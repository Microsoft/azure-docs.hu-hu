...
## <a name="im-having-an-issue-with-azure-file-sync-on-my-server-sync-cloud-tiering-etc-should-i-remove-and-recreate-my-server-endpoint"></a>Probléma adódnak a kiszolgáló Azure File Sync (szinkronizálás, felhőbeli rétegezés stb.) problémáim vannak. Távolítsam el, majd hozzam létre újra a kiszolgálóvégpontomat?
...
Ez az üzenet akkor jelenik meg, ha a kiszolgálót korábban regisztrálták egy társzinkronizálási szolgáltatásban. A kiszolgáló aktuális szinkronizálási szolgáltatásból való regisztrációjának, majd egy új szinkronizálási szolgáltatásban való regisztrálásának a megszinkronizálási szolgáltatásban való regisztrációjának a következő lépéseit kell végrehajtania: Kiszolgáló regisztrációjának [Azure File Sync.](storage-sync-files-server-registration.md#unregister-the-server-with-storage-sync-service)
...
> [!Warning]  
> Ha egy felhőbeli végpont által jelenleg használt Azure-fájlmegosztás metaadatait törli, a Azure File Sync sikertelen lesz. Ha ezt a fájlmegosztást egy másik szinkronizálási csoportban használja szinkronizálásra, a régi szinkronizálási csoportban lévő fájlok adatvesztése szinte biztos.
...
Ez a hiba a kiszolgálónkénti kiszolgálóvégpontok korlátjának elérésekor következik be. Az Azure File Sync jelenleg legfeljebb 30 kiszolgálóvégpontot támogat kiszolgálónként. További információkért lásd a [Azure File Sync célokat.](./storage-files-scale-targets.md#azure-file-sync-scale-targets)
...
- Ha a GetNextJob a következő állapottal fejeződött **be: -2134347756,** a kiszolgáló egy tűzfal, proxy vagy TLS-titkosítási csomag rendelési konfigurációja miatt nem tud kommunikálni az Azure File Sync szolgáltatással. 
    ...
    - Ha a kiszolgáló proxy mögött található, konfigurálja a gépre vagy az alkalmazásra vonatkozó proxybeállításokat a Proxy dokumentációjának [lépéseit követve.](./storage-sync-files-firewall-and-proxy.md#proxy)
...
| <ul><li>0x0001FFFE – 0x0001FFFF = 2 (nem jellemző)</li><li>0x0002FFFE – 0x0002FFFF = 2 (nem jellemző)</li><li>0x0003FFFE – 0x0003FFFF = 2 (nem jellemző)</li><li>0x0004FFFE – 0x0004FFFF = 2 (nem jellemző)</li><li>0x0005FFFE – 0x0005FFFF = 2 (nem jellemző)</li><li>0x0006FFFE – 0x0006FFFF = 2 (nem jellemző)</li><li>0x0007FFFE – 0x0007FFFF = 2 (nem jellemző)</li><li>0x0008FFFE – 0x0008FFFF = 2 (nem jellemző)</li><li>0x0009FFFE – 0x0009FFFF = 2 (nem jellemző)</li><li>0x000AFFFE – 0x000AFFFF = 2 (nem jellemző)</li><li>0x000BFFFE – 0x000BFFFF = 2 (nem jellemző)</li><li>0x000CFFFE – 0x000CFFFF = 2 (nem jellemző)</li><li>0x000DFFFE – 0x000DFFFF = 2 (nem jellemző)</li><li>0x000EFFFE – 0x000EFFFF = 2 (nincs definiálva)</li><li>0x000FFFFE – 0x000FFFFF = 2 (kiegészítő privát használatú terület)</li></ul> | 30 |
...
| **Szervizelésre van szükség** | Igen |
...
- Felhőbeli tárhelyhibák
    ...
    - *Nem elérhető Azure-fájlmegosztás.* Ez a hiba általában akkor fordul elő, ha törli az Azure-fájlmegosztást, amikor az még mindig felhőbeli végpont egy szinkronizálási csoportban.
    - *Nem elérhető tárfiók.* Ez a hiba általában akkor fordul elő, ha törli a tárfiókot, miközben az továbbra is rendelkezik egy Azure-fájlmegosztással, amely egy felhőbeli végpont a szinkronizálási csoportban. 
- Kiszolgálóhibák 
  ...
  - *Hiányzó, sérült vagy egyéb módon hibás újraelemzési pont.* Az újraelemzési pont egy speciális adatstruktúra egy fájlon, amely két részből áll:
    1. Egy újraelemzési címke, amely azt jelzi az operációs rendszer számára, hogy az Azure File Sync fájlrendszerszűrőnek (StorageSync.sys) valamilyen műveletet kell a fájl I/O-műveletével kell megtennie. 
...
1. A Eseménynapló tekintse át az Alkalmazások és szolgáltatások\Microsoft\FileSync\Agent mappában található telemetriai, működési és diagnosztikai eseménynaplókat.
    ...
    4. Ellenőrizze, hogy Azure File Sync -szűrőillesztők (StorageSync.sys és StorageSyncGuard.sys) futnak-e:
        - Egy rendszergazda jogú parancssorban futtassa a következőt: `fltmc` . Ellenőrizze, hogy StorageSync.sys és StorageSyncGuard.sys fájlrendszerszűrő-illesztőprogramok listában szerepelnek-e.

> [!NOTE]
> A 9006-os eseményazonosítót a rendszer óránként egyszer naplózza a telemetriai eseménynaplóban, ha egy fájl visszahívása sikertelen (hibakódonként egy eseményt naplóz). Tekintse meg [a Visszahívási hibák és szervizelés szakaszt,](#recall-errors-and-remediation) és ellenőrizze, hogy a hibakód tartalmazza-e a javítási lépéseket.
...
### <a name="how-to-troubleshoot-files-unexpectedly-recalled-on-a-server"></a>A kiszolgálón váratlanul előhívással elő visszahívott fájlok hibaelhárítása  
...
Ha a kiszolgálón Azure File Sync problémákba ütközik, kezdje a következő lépésekkel:
