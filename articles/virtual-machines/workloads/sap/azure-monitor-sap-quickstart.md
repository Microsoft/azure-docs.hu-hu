...
1. Válassza **Azure Monitor for SAP Solutions** lehetőséget **a Azure Marketplace** közül.
   ...
   :::image type="content" source="./media/azure-monitor-sap/azure-monitor-quickstart-1.png" alt-text="A képen az SAP Azure Monitor megoldások ajánlatának kiválasztása látható a Azure Marketplace." lightbox="./media/azure-monitor-sap/azure-monitor-quickstart-1.png":::
...
6. Módosítsa az érintett "service/protectedwebmethods" paraméter értékét "SDEFAULT -GetQueueStatistic –ABAPGetWPTable –EnqGetStatistic –GetProcessList" értékre az ajánlott beállításra, és kattintson a Másolás gombra
7. Vissza profil és a mentés >kiválasztása
8. A paraméter érvénybe lépő újraindítási rendszere

>[!Tip]
> A Access Control (ACL) használatával szűrheti a kiszolgálóporthoz való hozzáférést. Tekintse meg az [SAP-megjegyzését](https://launchpad.support.sap.com/#/notes/1495075)
...
3.  Adja meg az SAP-rendszer állomásnevét és az altartományt (ha van)
...
1. Válassza **a Szolgáltató** lapot a konfigurálni kívánt szolgáltatók hozzáadásához. A figyelési erőforrás üzembe helyezése után több szolgáltatót is hozzáadhat, vagy hozzáadhatja őket. 

   :::image type="content" source="./media/azure-monitor-sap/azure-monitor-quickstart-3.png" alt-text="Megjeleníti a szolgáltató lapját, amely további szolgáltatókat ad hozzá a Azure Monitor for SAP Solutions." lightbox="./media/azure-monitor-sap/azure-monitor-quickstart-3.png":::

2. Válassza **a Szolgáltató hozzáadása lehetőséget,** **SAP HANA** a legördülő menüből. 

   > [!IMPORTANT]
   > Győződjön meg SAP HANA, hogy a szolgáltató konfigurálva van SAP HANA főcsomóponthoz.

3. Adja meg a HANA-kiszolgáló magánhálózati IP-címét.

4. Adja meg a használni kívánt adatbázis-bérlő nevét. Bármilyen bérlőt választhat, javasoljuk azonban a **SYSTEMDB használatát,** mivel ez a monitorozási területek szélesebb skáláját teszi lehetővé. 

5. Adja meg a HANA-adatbázishoz társított SQL-portszámot. A portszámnak a következő formátumban kell lennie: **[3]**  +  **[példány#]**  +  **[13]**. Például: 30013. 
...
1. Válassza **a Magas rendelkezésre állású fürt (Pacemaker)** lehetőséget a legördülő menüből. 

   > [!IMPORTANT]
   > A magas rendelkezésre állású fürt (Pacemaker) szolgáltatójának konfigurálása érdekében győződjön meg arról, ha_cluster_provider minden csomóponton telepítve van. További információ: [HA-fürtök halmaza](https://github.com/ClusterLabs/ha_cluster_exporter#installation)

2. A Prometheus-végpontot a következő formában adja meg: http://IP:9664/metrics . 
...
1. Válassza az Operációs rendszer (Linux) lehetőséget a legördülő menüből 
   ...
   >[!IMPORTANT]
   > Az operációs rendszer (Linux) szolgáltatójának konfiguráláskor győződjön meg arról, hogy a Node_Exporter legújabb verziója telepítve van minden monitorni kívánt gazdagépen (BareMetal vagy VM). Ezzel a [hivatkozással megkeresi](https://prometheus.io/download/#node_exporter) a legújabb verziót. További információ: [Node_Exporter](https://github.com/prometheus/node_exporter)
...
3. Adja meg a csomópont-végpontot a következő formában: http://IP:9100/metrics .
   ...
   >Ha firewall-cmd: firewall-cmd --permanent --add-port=9100/tcp firewall-cmd --reload Ha az ufw: ufw allow 9100/tcp ufw reload
