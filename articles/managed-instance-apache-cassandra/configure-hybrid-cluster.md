...
1. Most alkalmazzuk a VNet és az alhálózatra vonatkozó speciális engedélyeket, amelyeket a Cassandra felügyelt példány igényel az Azure CLI használatával. Először is fel kell derítenie a `Resource ID` meglévő VNet. Másolja az érték kimenetét ebből a parancsból később, ez a következő: `Resource ID` .
...
1. Most alkalmazzuk a speciális engedélyeket, és az előző parancs kimenetét a hatókör paraméterként kell átadni:
   ...
   ```
    > [!NOTE]
    > A `assignee` `role` fenti és értékek rögzített szolgáltatási elv és szerepkör-azonosítók. 
