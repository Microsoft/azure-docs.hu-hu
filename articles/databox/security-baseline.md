...
description: A Azure Data Box biztonsági alapterve az Azure biztonsági Teljesítménytesztben meghatározott biztonsági javaslatok megvalósítására szolgáló eljárási útmutatást és erőforrásokat biztosít.
author: msmbaldwin
ms.service: databox
ms.topic: conceptual
ms.date: 02/17/2021
...
Ez a biztonsági alapkonfiguráció az [Azure biztonsági teljesítményteszt 1,0-es verziójáról](../security/benchmarks/overview-v1.md) Azure Data Boxre vonatkozó útmutatást alkalmaz. Az Azure Security Benchmark ajánlásokat ad arra nézve, hogy hogyan tehetők biztonságossá a felhőalapú megoldások az Azure-ban.
A tartalom az Azure biztonsági teljesítményteszt által meghatározott **biztonsági vezérlők** szerint van csoportosítva, valamint a Azure Data Box vonatkozó kapcsolódó útmutatás. A Azure Data Box nem alkalmazható **vezérlők** ki vannak zárva.

 
Ha szeretné megtekinteni, hogyan Azure Data Box teljes mértékben leképezni az Azure biztonsági Teljesítménytesztét, tekintse meg a [teljes Azure Data Box biztonsági alapterv-leképezési fájlt](https://github.com/MicrosoftDocs/SecurityBenchmarks/tree/master/Azure%20Offer%20Security%20Baselines).
...
Megadhatja, hogy ki férhet hozzá a rendeléshez a rendelés első létrehozásakor. Az Azure RBAC szerepkörei különböző hatókörökön állíthatók be a Data Boxi sorrendhez való hozzáférés szabályozása érdekében. Az Azure RBAC-szerepkör határozza meg a hozzáférés típusát – írható és olvasható, írásvédett, írás és olvasás a műveletek egy részhalmazára.
...
**Útmutató**: egy emelt szintű hozzáférési munkaállomás (Paw) használata az Azure ad többtényezős hitelesítéssel való bejelentkezéshez és a Azure Data Box-megrendelések konfigurálásához.
...
**Útmutató**: nem alkalmazható, Azure Data Box nem rendelkezik saját rendszergazdai fiókkal. A Azure Portalon keresztül férhet hozzá. Az Azure-előfizetését azonban beállíthatja úgy, hogy az Azure Active Directory (Azure AD) Privileged Identity Management (PIM) használja a naplók és riasztások generálásához, ha a környezetben gyanús vagy nem biztonságos tevékenység történik.
...
Emellett a Azure Data Box védi az eszköz zárolási kulcsát (más néven eszköz jelszavát), amely az eszköz titkosítási kulccsal történő zárolására szolgál. Alapértelmezés szerint egy Data Box-rendelés eszközének feloldási kulcsa egy Microsoft által felügyelt kulccsal van titkosítva. Az eszköz feloldási kulcsának további szabályozásához megadhatja az ügyfél által felügyelt kulcsot is. Az ügyfél által felügyelt kulcsokat egy Azure Key Vault kell létrehozni és tárolni.
