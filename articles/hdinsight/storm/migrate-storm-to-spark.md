...
:::image type="content" source="./media/migrate-storm-to-spark/storm-migration-path.png" alt-text="HDInsight Storm áttelepítési útvonala" border="false":::
...
:::image type="content" source="./media/migrate-storm-to-spark/streaming-and-micro-batch-processing.png" alt-text="folyamatos átvitel és mikro-kötegelt feldolgozás" border="false":::
...
:::image type="content" source="./media/migrate-storm-to-spark/apache-storm-components.png" alt-text="Storm-összetevők interakciója" border="false":::
...
:::image type="content" source="./media/migrate-storm-to-spark/nimbus-zookeeper-supervisor.png" alt-text="Nimbus, Zookeeper és felügyelő démonok" border="false":::
...
:::image type="content" source="./media/migrate-storm-to-spark/spark-streaming-to-output.png" alt-text="a Spark streaming elérési útja a kimenethez" border="false":::
...
:::image type="content" source="./media/migrate-storm-to-spark/spark-streaming-batches.png" alt-text="Spark streaming-feldolgozási kötegek" border="false":::
...
:::image type="content" source="./media/migrate-storm-to-spark/structured-streaming-data-processing.png" alt-text="az adatfeldolgozás strukturált adatfolyamban" border="false":::

:::image type="content" source="./media/migrate-storm-to-spark/structured-streaming-model.png" alt-text="programozási modell strukturált folyamatos átvitelhez" border="false":::
...
* A Storm által feldolgozott adattárolók (például az Azure Storage vagy a Azure Data Lake Storage Gen2) egy adatfogadóba íródnak.
   ...
   :::image type="content" source="./media/migrate-storm-to-spark/presumed-current-environment.png" alt-text="feltételezett aktuális környezet diagramja"  border="false":::
...
1. **Helyezzen üzembe egy új fürtöt.** Helyezzen üzembe egy új HDInsight 4,0 Spark-fürtöt ugyanabban a virtuális hálózatban, és végezze el a Spark streaming vagy a Spark strukturált streaming-alkalmazás üzembe helyezését, és tesztelje alaposan.
   ...
   :::image type="content" source="./media/migrate-storm-to-spark/new-spark-deployment.png" alt-text="új Spark-telepítés a HDInsight-ben" border="false":::
...
1. **A régi Storm-fürtön való fogyasztás leállítása.** A meglévő Storm-ben állítsa le az adatok átvitelét a streaming adatforrásból, és várjon, amíg az adatok befejeződik a cél fogadóba való írás.
   ...
   :::image type="content" source="./media/migrate-storm-to-spark/stop-consuming-current-cluster.png" alt-text="az aktuális fürtön való felhasználás leállítása" border="false":::
...
1. **Az új Spark-fürtön való használat megkezdése.** Egy újonnan telepített HDInsight 4,0 Spark-fürtről származó adatfolyam-adatok indítása. Ebben az időben a folyamat a legújabb Kafka-eltoláson keresztül történik.
   ...
   :::image type="content" source="./media/migrate-storm-to-spark/start-consuming-new-cluster.png" alt-text="használat megkezdése az új fürtön" border="false":::
...
1. **Szükség szerint távolítsa el a régi fürtöt.** Ha a kapcsoló elkészült és megfelelően működik, távolítsa el a régi HDInsight 3,6 Storm-fürtöt szükség szerint.
   ...
   :::image type="content" source="./media/migrate-storm-to-spark/remove-old-clusters1.png" alt-text="szükség szerint távolítsa el a régi HDInsight-fürtöket" border="false":::
